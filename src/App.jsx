...
import { 
  ...
  TrendingDown, Gift, Crosshair, Flame, UsersRound, LineChart, ClipboardList, AlertTriangle, Info, Edit, Store, Send, Eye, EyeOff, MessageSquarePlus, ShieldAlert, Share2, MessageCircleHeart, Coffee, CalendarDays
} from 'lucide-react';
...
const INITIAL_PRODUCTS = [
  { id: 'prod-1', name: 'Cookie Tradicional', price: 10.00, type: 'single', units: 1, isVisible: true, description: 'Cookie clássico com gotas de chocolate belga.' }
];
...
export default function App() {
  ...
  const [storeSettings, setStoreSettings] = useState({ 
    isStoreOpen: true, 
    closedMessage: 'Estamos preparando novidades no forno! Voltamos logo.', 
    maxAdvanceDays: 14, 
    whatsappNumber: '' 
  });
  ...
  const [newProduct, setNewProduct] = useState({ name: '', price: '', type: 'single', units: 2, description: '' });
  ...
  const [checkoutData, setCheckoutData] = useState({ name: '', phone: '', referredBy: '', date: getTodayYMD(), deliveryType: 'unesp', period: 'Manhã (Departamentos/Prédios)', address: '', orderObservation: '', acceptedPolicies: false });
  ...
  const handleProduce = () => {
    ...
    alert("Produção registrada com sucesso! O estoque foi deduzido.");
  };
  ...
  const handleApproveOnlineOrder = (order) => {
    order.cart.forEach(item => {
      ...
      saveToDb('reservations', newId, {
          ...
          observation: `[Loja - ${order.deliveryType === 'unesp' ? 'UNESP ' + order.period : 'Casa ' + order.address}] ${order.orderObservation || ''}`
      });
    });
  ...
  const handleAddProduct = (e) => {
    ...
    setNewProduct({ name: '', price: '', type: 'single', units: 2, description: '' });
  };
  ...
  // ==========================================
  // RENDERIZAÇÃO: LOJA ONLINE PÚBLICA (STOREFRONT - THEME EMURAL)
  // ==========================================
  ...
  const handleAddToCartPublic = (product) => {
    setStoreCart(prev => {
       ...
       return [...prev, { productId: product.id, name: product.name, price: product.price, units: product.units || 1, qty: 1 }];
    });
  ...
  const handleCheckoutPublic = async (e) => {
    ...
    const orderPayload = {
       ...
       cart: storeCart,
       total: total,
       orderObservation: checkoutData.orderObservation,
    ...
    try {
      ...
      if (checkoutData.orderObservation) {
         msg += `\n*Observações do Pedido:* ${checkoutData.orderObservation}\n`;
      }
      ...
      setStoreCart([]); 
      setCheckoutData(prev => ({...prev, orderObservation: '', acceptedPolicies: false}));
      setIsCartOpen(false);
      alert('Seu pedido foi enviado para o nosso forno e também abriu no seu WhatsApp! Envie a mensagem para confirmar.');
  ...
  if (appMode === 'loading') {
    return (
      <div className="min-h-screen bg-[#FFF8F0] dark:bg-[#221510] flex flex-col items-center justify-center p-4">
        <Store className="text-amber-800 dark:text-[#C17F59] animate-pulse mb-4" size={48} />
        <p className="text-amber-900 dark:text-[#F3E8D6] font-medium">A aquecer o forno...</p>
      </div>
    );
  }
  ...
  if (appMode === 'storefront') {
    return (
      <div className={`min-h-screen font-sans pb-24 transition-colors duration-300 ${darkMode ? 'dark bg-[#221510] text-[#F3E8D6]' : 'bg-[#FFF8F0] text-amber-950'}`}>
         {/* CABEÇALHO */}
         <header className="bg-amber-800 dark:bg-[#3D2C20] text-white p-4 sticky top-0 z-40 shadow-sm transition-colors border-b border-amber-900 dark:border-[#4A3B32]">
            <div className="max-w-5xl mx-auto flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
               <div className="flex items-center justify-between w-full sm:w-auto">
                 <div className="flex items-center gap-2">
                   <Store size={28} className="text-amber-200 dark:text-[#C17F59]" />
                   <h1 className="text-2xl font-black tracking-tight drop-shadow-sm text-white dark:text-[#F3E8D6]">Emural</h1>
                 </div>
                 
                 {/* Controles Mobile Visíveis em Cima */}
                 <div className="flex items-center gap-3 sm:hidden">
                   <button onClick={() => { if(user && !user.isAnonymous) setAppMode('dashboard'); else setAppMode('admin_login'); }} className="text-[10px] font-bold text-amber-100 hover:text-white border border-amber-100 dark:border-[#4A3B32] px-2 py-1 rounded">
                     {user && !user.isAnonymous ? 'Painel' : 'Login'}
                   </button>
                   <button onClick={() => setDarkMode(!darkMode)} className="text-amber-100 dark:text-[#C17F59] hover:text-white dark:hover:text-[#F3E8D6] transition-colors p-1">
                     {darkMode ? <Sun size={20} /> : <Moon size={20} />}
                   </button>
                   <div className="relative">
                     <button onClick={() => setIsCartOpen(true)} className="relative flex items-center gap-2 bg-black/20 dark:bg-[#2C1E16] hover:bg-black/30 px-3 py-1.5 rounded-full transition-colors font-bold text-sm">
                       <ShoppingBag size={18} />
                       {storeCart.length > 0 && <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] w-5 h-5 flex items-center justify-center rounded-full border-2 border-amber-800 dark:border-[#3D2C20]">{storeCart.reduce((a,b)=>a+b.qty,0)}</span>}
                     </button>
                     {showCartToast && (
                        <div className="absolute top-full mt-2 right-0 bg-green-600 text-white text-xs px-2 py-1 rounded shadow-lg animate-bounce whitespace-nowrap z-50">
                          Adicionado!
                        </div>
                     )}
                   </div>
               ...
               <div className="flex gap-2 sm:gap-4 overflow-x-auto pb-2 sm:pb-0 scrollbar-hide w-full sm:w-auto">
                  <button onClick={() => setPublicTab('store')} className={`font-bold px-4 py-2 rounded-full whitespace-nowrap transition-colors flex-1 sm:flex-auto flex items-center justify-center gap-2 ${publicTab === 'store' ? 'bg-white text-amber-900 dark:bg-[#2C1E16] dark:text-[#C17F59] shadow-sm' : 'text-amber-100 dark:text-[#E2D4C1] hover:bg-white/20 dark:hover:bg-[#4A3B32]'}`}>
                    <Store size={18} /> Cardápio
                  </button>
                  <button onClick={() => setPublicTab('community')} className={`font-bold px-4 py-2 rounded-full whitespace-nowrap transition-colors flex-1 sm:flex-auto flex items-center justify-center gap-2 ${publicTab === 'community' ? 'bg-white text-amber-900 dark:bg-[#2C1E16] dark:text-[#C17F59] shadow-sm' : 'text-amber-100 dark:text-[#E2D4C1] hover:bg-white/20 dark:hover:bg-[#4A3B32]'}`}>
                    <MessageSquarePlus size={18} /> Mural
                  </button>
               </div>

               {/* Controles Desktop */}
               <div className="hidden sm:flex items-center gap-3">
                 <button onClick={() => { if(user && !user.isAnonymous) setAppMode('dashboard'); else setAppMode('admin_login'); }} className="text-xs font-bold text-amber-100 hover:text-white transition-colors border border-amber-100 dark:border-[#4A3B32] px-3 py-1.5 rounded-lg">
                 ...
                 <div className="relative">
                   <button onClick={() => setIsCartOpen(true)} className="relative flex items-center gap-2 bg-black/20 dark:bg-[#2C1E16] hover:bg-black/30 px-4 py-2 rounded-full transition-colors font-bold text-sm">
                     <ShoppingBag size={18} /> <span>Carrinho</span>
                     {storeCart.length > 0 && <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] w-5 h-5 flex items-center justify-center rounded-full border-2 border-amber-800 dark:border-[#3D2C20]">{storeCart.reduce((a,b)=>a+b.qty,0)}</span>}
                   </button>
                   {showCartToast && (
                      <div className="absolute top-full mt-2 right-0 bg-green-600 text-white text-xs px-3 py-1.5 rounded-lg shadow-lg animate-bounce whitespace-nowrap z-50 flex items-center gap-1">
         ...
         <main className="max-w-5xl mx-auto p-4 mt-6 text-center">
            ...
            {!publicSettings.isStoreOpen ? (
               <div className="bg-white dark:bg-[#3D2C20] p-8 rounded-3xl shadow-sm text-center border border-amber-200 dark:border-[#4A3B32] mt-10 transition-colors">
                  <Coffee size={64} className="text-amber-800 dark:text-[#C17F59] mx-auto mb-4" />
                  <h2 className="text-2xl font-bold text-amber-950 dark:text-[#F3E8D6] mb-2">Forno Fechado no Momento</h2>
                  <p className="text-stone-600 dark:text-[#E2D4C1] max-w-md mx-auto">{publicSettings.closedMessage}</p>
               </div>
            ) : (
               <>
                 {/* ABA 1: CATÁLOGO */}
                 {publicTab === 'store' && (
                   <section className="animate-in fade-in duration-300 text-left">
                     <div className="text-center mb-8">
                       <h2 className="text-3xl sm:text-4xl font-black text-amber-950 dark:text-[#F3E8D6] mb-2 drop-shadow-sm flex justify-center items-center gap-3">
                         <Coffee className="w-8 h-8 text-amber-800 dark:text-[#C17F59] hidden sm:block" />
                         Nossos Produtos
                       </h2>
                       <p className="text-amber-800 dark:text-[#E2D4C1] font-medium">Conheça o cardápio e faça a sua reserva.</p>
                     </div>

                     <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                        {publicProducts.map(prod => (
                           <div key={prod.id} className="bg-white dark:bg-[#3D2C20] rounded-2xl shadow-sm border border-amber-100 dark:border-[#4A3B32] overflow-hidden flex flex-col transition-transform hover:-translate-y-1">
                              <div className="p-5 flex flex-col flex-1">
                                 <h3 className="font-bold text-lg text-amber-950 dark:text-[#F3E8D6] mb-1 leading-tight">{prod.name}</h3>
                                 {prod.type === 'combo' && <p className="text-xs text-amber-700 dark:text-[#C17F59] font-bold mb-2">{prod.units} unidades</p>}
                                 
                                 {prod.description && (
                                   <p className="text-sm text-stone-500 dark:text-stone-400 mb-4 mt-1 flex-1">
                                     {prod.description}
                                   </p>
                                 )}

                                 <p className="text-xl font-black text-amber-900 dark:text-[#F3E8D6] mb-4 mt-auto border-t border-amber-50 dark:border-[#4A3B32] pt-4">
                                    R$ {(Number(prod.price)||0).toFixed(2)}
                                 </p>
                                 
                                 <button onClick={() => handleAddToCartPublic(prod)} className="w-full bg-amber-800 hover:bg-amber-900 dark:bg-[#C17F59] dark:hover:bg-[#A66A4B] text-white dark:text-[#2C1E16] font-bold py-3 rounded-xl transition-colors flex justify-center items-center gap-2 shadow-sm mt-auto">
                     ...
                     <div className="mt-16 mb-8 text-center opacity-80">
                        <p className="text-xl font-black text-amber-800 dark:text-[#C17F59] mb-1">Aguarde...</p>
                        <p className="text-sm font-medium text-stone-600 dark:text-stone-400">Mais novidades deliciosas estão a caminho do nosso forno! ✨</p>
                     </div>
                   </section>
                 )}
                 ...
                 {publicTab === 'community' && (
                   <section className="animate-in fade-in duration-300 space-y-8">
                     ...
                     {/* SESSÃO: INDIQUE E GANHE */}
                     <div className="bg-gradient-to-br from-amber-800 to-amber-950 dark:from-[#3D2C20] dark:to-[#2C1E16] border border-amber-900/50 dark:border-[#4A3B32] rounded-3xl p-8 shadow-md relative overflow-hidden text-left text-white">
                       ...
                       <div className="flex items-center gap-3 mb-4 relative z-10">
                         <h2 className="text-2xl font-bold text-white flex items-center gap-2">
                           <Share2 size={24} /> Indique e Ganhe!
                         </h2>
                       </div>
                       
                       <p className="text-amber-100 dark:text-[#E2D4C1] mb-6 max-w-lg leading-relaxed relative z-10">
                         Compartilhe a loja com amigos. A cada indicação que realizar uma compra (usando o seu nome), você ganha recompensas para usar com a gente.
                       </p>
                       ...
                       <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10 mb-6">
                         <div className="flex items-center gap-3 p-4 rounded-xl bg-white/10 backdrop-blur-sm border border-white/10">
                           ...
                           <div>
                             <h3 className="font-bold text-white mb-1">1. Compartilhe</h3>
                             <p className="text-xs text-amber-100/80">Espalhe para os amigos.</p>
                           </div>
                         </div>
                         <div className="flex items-center gap-3 p-4 rounded-xl bg-white/10 backdrop-blur-sm border border-white/10">
                           <ShoppingBag className="text-amber-200" size={24} />
                           <div>
                             <h3 className="font-bold text-white mb-1">2. Eles Pedem</h3>
                             <p className="text-xs text-amber-100/80">Com seu nome na indicação.</p>
                           </div>
                         </div>
                         <div className="flex items-center gap-3 p-4 rounded-xl bg-white/10 backdrop-blur-sm border border-white/10">
                           <Gift className="text-amber-200" size={24} />
                           <div>
                             <h3 className="font-bold text-white mb-1">3. Você Ganha!</h3>
                             <p className="text-xs text-amber-100/80">Produtos gratuitos.</p>
                           </div>
                         </div>
                       </div>
                     </div>

                     {/* SESSÃO: EVENTOS E DESAFIOS */}
                     <div className="bg-white dark:bg-[#3D2C20] border border-amber-100 dark:border-[#4A3B32] rounded-3xl p-8 shadow-sm text-left">
                       <h2 className="text-xl font-bold text-amber-950 dark:text-[#F3E8D6] mb-4 flex items-center gap-2">
                         <CalendarDays className="w-6 h-6 text-amber-700 dark:text-[#C17F59]" /> Eventos e Desafios
                       </h2>
                       <p className="text-stone-600 dark:text-stone-400 mb-6">
                         Fique atento às nossas missões especiais e eventos por tempo limitado. Participe para acumular recompensas e experimentar novidades exclusivas.
                       </p>
                       <div className="bg-amber-50 dark:bg-[#2C1E16] p-5 rounded-2xl border border-dashed border-amber-300 dark:border-[#5C4535] flex items-start gap-4">
                         <div className="bg-amber-200 dark:bg-[#4A3B32] p-3 rounded-full shrink-0">
                           <Target className="w-6 h-6 text-amber-800 dark:text-[#C17F59]" />
                         </div>
                         <div>
                           <h4 className="font-semibold text-amber-900 dark:text-[#F3E8D6] mb-1">Nenhum evento ativo no momento</h4>
                           <p className="text-sm text-stone-500 dark:text-stone-400">
                             Estamos preparando surpresas no forno. Volte em breve para conferir os próximos desafios!
                           </p>
                         </div>
                       </div>
                     </div>
                     ...
                     <div className="bg-white dark:bg-[#3D2C20] p-6 sm:p-8 rounded-3xl shadow-sm border border-amber-100 dark:border-[#4A3B32] transition-colors text-left">
                        <h2 className="text-xl font-bold text-amber-950 dark:text-[#F3E8D6] mb-2 flex items-center gap-2">
                          <MessageSquarePlus className="w-6 h-6 text-amber-700 dark:text-[#C17F59]" /> Mural de Ideias & Feedback
                        </h2>
                        <p className="text-stone-600 dark:text-stone-400 mb-6">
                          Gostaríamos muito de ouvir você. Tem alguma sugestão de novo produto, melhoria ou comentário sobre o que provou?
                        </p>
                        
                        {/* TABS PARA O FORM */}
                        <div className="flex gap-2 p-1 bg-amber-50 dark:bg-[#2C1E16] rounded-xl mb-6">
                          <button onClick={() => setPublicFormType('idea')} className={`flex-1 py-2.5 font-bold text-sm rounded-lg transition-all ${publicFormType === 'idea' ? 'bg-white shadow-sm text-amber-900 dark:bg-[#3D2C20] dark:text-[#F3E8D6]' : 'text-stone-500 hover:bg-white/50 dark:text-stone-400'}`}>Nova Ideia</button>
                          <button onClick={() => setPublicFormType('feedback')} className={`flex-1 py-2.5 font-bold text-sm rounded-lg transition-all ${publicFormType === 'feedback' ? 'bg-white shadow-sm text-amber-900 dark:bg-[#3D2C20] dark:text-[#F3E8D6]' : 'text-stone-500 hover:bg-white/50 dark:text-stone-400'}`}>Dar Feedback</button>
                        </div>

                        {publicFormType === 'idea' ? (
                           <form onSubmit={handleSendPublicSuggestion} className="flex flex-col gap-4 animate-in fade-in">
                             <div>
                                <label className="block text-xs font-bold text-amber-900 dark:text-[#E2D4C1] uppercase mb-1">Seu Nome *</label>
                                <input required type="text" className="w-full p-3 bg-amber-50/50 dark:bg-[#2C1E16] border border-amber-200 dark:border-[#4A3B32] rounded-xl outline-none text-sm text-amber-950 dark:text-[#F3E8D6] focus:ring-2 focus:ring-amber-500 dark:focus:ring-[#C17F59]" value={pubSugData.name} onChange={e => setPubSugData({...pubSugData, name: e.target.value})} placeholder="Como se chama?" />
                             </div>
                             <div>
                                <label className="block text-xs font-bold text-amber-900 dark:text-[#E2D4C1] uppercase mb-1">Sua Ideia *</label>
                                <input required type="text" className="w-full p-3 bg-amber-50/50 dark:bg-[#2C1E16] border border-amber-200 dark:border-[#4A3B32] rounded-xl outline-none text-sm text-amber-950 dark:text-[#F3E8D6] focus:ring-2 focus:ring-amber-500 dark:focus:ring-[#C17F59]" value={pubSugData.text} onChange={e => setPubSugData({...pubSugData, text: e.target.value})} placeholder="Ex: Produto de Pistache!" />
                             </div>
                             <div className="flex flex-col sm:flex-row gap-4 mt-2">
                               <div className="flex-1">
                                  <label className="block text-xs font-bold text-amber-900 dark:text-[#E2D4C1] uppercase mb-1">Categoria *</label>
                                  <select className="w-full p-3 bg-amber-50/50 dark:bg-[#2C1E16] border border-amber-200 dark:border-[#4A3B32] rounded-xl outline-none text-sm text-amber-950 dark:text-[#F3E8D6] focus:ring-2 focus:ring-amber-500 dark:focus:ring-[#C17F59]" value={pubSugData.type} onChange={e => setPubSugData({...pubSugData, type: e.target.value})}>
                               ...
                               <button type="submit" className="w-full sm:w-auto bg-amber-800 hover:bg-amber-900 text-white dark:bg-[#C17F59] dark:hover:bg-[#A66A4B] dark:text-[#2C1E16] font-bold py-3 px-8 rounded-xl transition mt-auto h-[46px] shadow-sm flex items-center justify-center gap-2">
                           ...
                           <form onSubmit={handleSendPublicFeedback} className="flex flex-col gap-4 animate-in fade-in">
                             <div className="flex flex-col sm:flex-row gap-4">
                               <div className="flex-1">
                                  <label className="block text-xs font-bold text-amber-900 dark:text-[#E2D4C1] uppercase mb-1">Seu Nome *</label>
                                  <input required type="text" className="w-full p-3 bg-amber-50/50 dark:bg-[#2C1E16] border border-amber-200 dark:border-[#4A3B32] rounded-xl outline-none text-sm text-amber-950 dark:text-[#F3E8D6] focus:ring-2 focus:ring-amber-500 dark:focus:ring-[#C17F59]" value={pubFeedbackData.name} onChange={e => setPubFeedbackData({...pubFeedbackData, name: e.target.value})} placeholder="Seu nome" />
                               </div>
                               <div className="flex-1">
                                  <label className="block text-xs font-bold text-amber-900 dark:text-[#E2D4C1] uppercase mb-1">Produto Avaliado *</label>
                                  <input required type="text" className="w-full p-3 bg-amber-50/50 dark:bg-[#2C1E16] border border-amber-200 dark:border-[#4A3B32] rounded-xl outline-none text-sm text-amber-950 dark:text-[#F3E8D6] focus:ring-2 focus:ring-amber-500 dark:focus:ring-[#C17F59]" value={pubFeedbackData.product} onChange={e => setPubFeedbackData({...pubFeedbackData, product: e.target.value})} placeholder="Ex: Produto Tradicional" />
                               </div>
                             </div>
                             <div className="flex-1 mt-2">
                                <label className="block text-xs font-bold text-amber-900 dark:text-[#E2D4C1] uppercase mb-1">Seu Feedback *</label>
                                <textarea required className="w-full p-4 bg-amber-50/50 dark:bg-[#2C1E16] border border-amber-200 dark:border-[#4A3B32] rounded-2xl outline-none text-sm text-amber-950 dark:text-[#F3E8D6] focus:ring-2 focus:ring-amber-500 dark:focus:ring-[#C17F59] min-h-[120px] resize-none" value={pubFeedbackData.text} onChange={e => setPubFeedbackData({...pubFeedbackData, text: e.target.value})} placeholder="Estava incrível! A textura da massa era..." />
                             </div>
                             <div className="flex flex-col sm:flex-row sm:items-center gap-4 justify-between mt-2">
                                <label className="flex items-center gap-2 cursor-pointer">
                                  <input type="checkbox" className="w-4 h-4 rounded border-gray-300 accent-amber-800 dark:accent-[#C17F59] cursor-pointer" checked={pubFeedbackData.allowRepublish} onChange={e => setPubFeedbackData({...pubFeedbackData, allowRepublish: e.target.checked})} />
                                  <span className="text-xs font-bold text-stone-600 dark:text-stone-400">Autorizo o uso do feedback no mural público</span>
                                </label>
                                <button type="submit" className="w-full sm:w-auto bg-amber-800 hover:bg-amber-900 text-white dark:bg-[#C17F59] dark:hover:bg-[#A66A4B] dark:text-[#2C1E16] font-bold py-3 px-6 rounded-xl transition shadow-sm flex items-center justify-center gap-2">
                                  <CheckCircle2 size={16} /> Enviar Feedback
                                </button>
                             </div>
                           </form>
                        )}
         ...
         {isCartOpen && (
            <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm z-50 flex justify-end">
               <div className="bg-[#FFF8F0] dark:bg-[#221510] w-full max-w-md h-full shadow-2xl flex flex-col animate-in slide-in-from-right-8 duration-300">
                  <div className="p-4 border-b border-amber-200 dark:border-[#4A3B32] flex justify-between items-center bg-white dark:bg-[#3D2C20]">
                    <h3 className="font-bold text-amber-950 dark:text-[#F3E8D6] flex items-center gap-2"><ShoppingBag size={20} className="text-amber-800 dark:text-[#C17F59]"/> Seu Pedido</h3>
                    <button onClick={() => setIsCartOpen(false)} className="text-stone-500 hover:text-amber-900 dark:text-stone-400 dark:hover:text-white p-2 bg-amber-50 dark:bg-[#2C1E16] rounded-full shadow-sm transition-colors"><X size={20} /></button>
                  </div>
                  
                  <div className="flex-1 overflow-y-auto p-4 space-y-4">
                     {storeCart.length === 0 ? (
                        <p className="text-center text-stone-500 dark:text-stone-400 mt-10 font-medium">Seu carrinho está vazio.</p>
                     ) : (
                        storeCart.map((item, i) => (
                           <div key={i} className="bg-white dark:bg-[#3D2C20] p-4 rounded-2xl shadow-sm border border-amber-100 dark:border-[#4A3B32] flex justify-between items-center transition-colors">
                              <div>
                                 <p className="font-bold text-amber-950 dark:text-[#F3E8D6] text-sm mb-1">{item.name}</p>
                                 <p className="text-xs font-black text-amber-800 dark:text-[#C17F59]">R$ {(item.price * item.qty).toFixed(2)}</p>
                              </div>
                              <div className="flex items-center gap-3">
                                 <div className="flex items-center gap-2 bg-amber-50 dark:bg-[#2C1E16] rounded-lg p-1 border border-amber-200 dark:border-[#4A3B32] shadow-sm">
                                   <button onClick={() => setStoreCart(prev => prev.map((p, idx) => idx === i ? {...p, qty: Math.max(1, p.qty - 1)} : p))} className="w-6 h-6 flex items-center justify-center font-bold text-stone-600 dark:text-stone-400 hover:bg-white dark:hover:bg-[#3D2C20] rounded-md">-</button>
                                   <span className="text-xs font-bold w-4 text-center text-amber-950 dark:text-[#F3E8D6]">{item.qty}</span>
                                   <button onClick={() => setStoreCart(prev => prev.map((p, idx) => idx === i ? {...p, qty: p.qty + 1} : p))} className="w-6 h-6 flex items-center justify-center font-bold text-stone-600 dark:text-stone-400 hover:bg-white dark:hover:bg-[#3D2C20] rounded-md">+</button>
                                 </div>
                     ...
                     {storeCart.length > 0 && (
                        <form id="checkout-form" onSubmit={handleCheckoutPublic} className="bg-white dark:bg-[#3D2C20] p-5 rounded-2xl shadow-sm border border-amber-200 dark:border-[#4A3B32] mt-6 space-y-4">
                           <h4 className="font-bold text-amber-950 dark:text-[#E2D4C1] border-b border-amber-100 dark:border-[#4A3B32] pb-2 mb-4">Finalizar Entrega</h4>
                           
                           {/* OBSERVAÇÕES ÚNICAS NO CARRINHO */}
                           <div>
                             <label className="block text-xs font-bold text-amber-900 dark:text-[#E2D4C1] uppercase mb-1">Observações do Pedido</label>
                             <textarea 
                               className="w-full p-3 bg-amber-50/50 dark:bg-[#2C1E16] border border-amber-200 dark:border-[#4A3B32] rounded-xl outline-none focus:ring-2 focus:ring-amber-500 dark:focus:ring-[#C17F59] text-sm text-amber-950 dark:text-[#F3E8D6] h-20 resize-none" 
                               value={checkoutData.orderObservation} 
                               onChange={e => setCheckoutData({...checkoutData, orderObservation: e.target.value})} 
                               placeholder="Ex: Retirar avelãs, embalar para presente..." 
                             />
                           </div>

                           <div className="pt-2">
                             <label className="block text-xs font-bold text-amber-900 dark:text-[#E2D4C1] uppercase mb-1">Seu Nome *</label>
                             <input required type="text" className="w-full p-3 bg-amber-50/50 dark:bg-[#2C1E16] border border-amber-200 dark:border-[#4A3B32] rounded-xl outline-none focus:ring-2 focus:ring-amber-500 dark:focus:ring-[#C17F59] text-sm text-amber-950 dark:text-[#F3E8D6]" value={checkoutData.name} onChange={e => setCheckoutData({...checkoutData, name: e.target.value})} placeholder="Como podemos te chamar?" />
                           </div>

                           <div>
                             <label className="block text-xs font-bold text-amber-900 dark:text-[#E2D4C1] uppercase mb-1">Seu WhatsApp / Telefone *</label>
                             <input required type="tel" className="w-full p-3 bg-amber-50/50 dark:bg-[#2C1E16] border border-amber-200 dark:border-[#4A3B32] rounded-xl outline-none focus:ring-2 focus:ring-amber-500 dark:focus:ring-[#C17F59] text-sm text-amber-950 dark:text-[#F3E8D6]" value={checkoutData.phone} onChange={e => setCheckoutData({...checkoutData, phone: e.target.value})} placeholder="(XX) 99999-9999" />
                           </div>

                           <div>
                             <label className="block text-xs font-bold text-amber-900 dark:text-[#E2D4C1] uppercase mb-1">Quem te indicou? (Opcional)</label>
                             <input type="text" className="w-full p-3 bg-amber-50/50 dark:bg-[#2C1E16] border border-amber-200 dark:border-[#4A3B32] rounded-xl outline-none focus:ring-2 focus:ring-amber-500 dark:focus:ring-[#C17F59] text-sm text-amber-950 dark:text-[#F3E8D6]" value={checkoutData.referredBy} onChange={e => setCheckoutData({...checkoutData, referredBy: e.target.value})} placeholder="Nome de quem te indicou" />
                           </div>

                           <div className="flex gap-3">
                             <div className="w-1/2">
                               <label className="block text-xs font-bold text-amber-900 dark:text-[#E2D4C1] uppercase mb-1">Entrega *</label>
                               <select required className="w-full p-3 bg-amber-50/50 dark:bg-[#2C1E16] border border-amber-200 dark:border-[#4A3B32] rounded-xl outline-none focus:ring-2 focus:ring-amber-500 dark:focus:ring-[#C17F59] text-sm text-amber-950 dark:text-[#F3E8D6]" value={checkoutData.deliveryType} onChange={e => setCheckoutData({...checkoutData, deliveryType: e.target.value})}>
                                 <option value="unesp">Na UNESP</option>
                                 <option value="home">Em Casa</option>
                               </select>
                             </div>
                             <div className="w-1/2">
                               <label className="block text-xs font-bold text-amber-900 dark:text-[#E2D4C1] uppercase mb-1">Data *</label>
                               <input required type="date" min={getTodayYMD()} max={getMaxDateYMD(publicSettings.maxAdvanceDays)} className="w-full p-3 bg-amber-50/50 dark:bg-[#2C1E16] border border-amber-200 dark:border-[#4A3B32] rounded-xl outline-none focus:ring-2 focus:ring-amber-500 dark:focus:ring-[#C17F59] text-sm text-amber-950 dark:text-[#F3E8D6] [&::-webkit-calendar-picker-indicator]:dark:invert" value={checkoutData.date} onChange={e => setCheckoutData({...checkoutData, date: e.target.value})} />
                             </div>
                           </div>

                           {checkoutData.deliveryType === 'unesp' && (
                             <div>
                               <label className="block text-xs font-bold text-amber-900 dark:text-[#E2D4C1] uppercase mb-1">Período/Local (UNESP) *</label>
                               <select required className="w-full p-3 bg-amber-50/50 dark:bg-[#2C1E16] border border-amber-200 dark:border-[#4A3B32] rounded-xl outline-none focus:ring-2 focus:ring-amber-500 dark:focus:ring-[#C17F59] text-sm text-amber-950 dark:text-[#F3E8D6]" value={checkoutData.period} onChange={e => setCheckoutData({...checkoutData, period: e.target.value})}>
                           ...
                           {checkoutData.deliveryType === 'home' && (
                             <div>
                               <label className="block text-xs font-bold text-amber-900 dark:text-[#E2D4C1] uppercase mb-1">Endereço Completo *</label>
                               <input required type="text" className="w-full p-3 bg-amber-50/50 dark:bg-[#2C1E16] border border-amber-200 dark:border-[#4A3B32] rounded-xl outline-none focus:ring-2 focus:ring-amber-500 dark:focus:ring-[#C17F59] text-sm text-amber-950 dark:text-[#F3E8D6]" value={checkoutData.address} onChange={e => setCheckoutData({...checkoutData, address: e.target.value})} placeholder="Rua, Número, Bairro" />
                             </div>
                           )}
                           ...
                           <div className="bg-red-50 dark:bg-red-900/20 p-4 rounded-xl border border-red-100 dark:border-red-800/50 flex flex-col gap-3 mt-4">
                             <div className="flex gap-2">
                               ...
                               <div>
                                 ...
                                 <ul className="text-[10px] text-red-800 dark:text-red-300 list-disc pl-4 mt-1 space-y-0.5">
                                   ...
                                   <li>Os pedidos são feitos <b>sob demanda</b> e sujeitos à disponibilidade.</li>
                             ...
                             <label className="flex items-center gap-2 cursor-pointer pt-2 border-t border-red-200/50 dark:border-red-800/50">
                               <input type="checkbox" required className="w-4 h-4 rounded border-gray-300 accent-amber-800 dark:accent-[#C17F59] cursor-pointer" checked={checkoutData.acceptedPolicies} onChange={e => setCheckoutData({...checkoutData, acceptedPolicies: e.target.checked})} />
                               <span className="text-xs font-bold text-stone-700 dark:text-stone-300">Li e concordo com os avisos.</span>
                             </label>
                           </div>
                        </form>
                     )}
                  ...
                  {storeCart.length > 0 && (
                     <div className="p-5 border-t border-amber-200 dark:border-[#4A3B32] bg-amber-50 dark:bg-[#302118]">
                        <div className="flex justify-between items-center mb-4">
                           <span className="font-bold text-amber-900 dark:text-[#E2D4C1]">Total do Pedido:</span>
                           <span className="font-black text-2xl text-amber-950 dark:text-[#F3E8D6]">R$ {storeCart.reduce((a,b)=>a+b.price*b.qty, 0).toFixed(2)}</span>
                        </div>
                        <button form="checkout-form" disabled={!checkoutData.acceptedPolicies} type="submit" className="w-full bg-amber-800 hover:bg-amber-900 dark:bg-[#C17F59] dark:hover:bg-[#A66A4B] disabled:bg-gray-300 dark:disabled:bg-gray-700 disabled:text-gray-500 disabled:cursor-not-allowed text-white dark:text-[#2C1E16] font-bold py-3.5 rounded-xl transition-colors shadow-sm flex items-center justify-center gap-2 text-lg">
         ...
         <footer className="mt-12 text-center text-sm text-stone-500 dark:text-stone-500 py-6">
            <p>Preparado com carinho no forno. &copy; {new Date().getFullYear()}</p>
         </footer>
      </div>
    );
  }
  ...
  if (appMode === 'admin_login') {
    return (
      <div className="min-h-screen bg-[#FFF8F0] dark:bg-[#221510] flex items-center justify-center p-4 transition-colors">
        <div className="bg-white dark:bg-[#3D2C20] p-8 rounded-3xl shadow-xl border border-amber-100 dark:border-[#4A3B32] w-full max-w-md relative">
          <button onClick={() => setAppMode('storefront')} className="absolute top-4 right-4 text-stone-400 hover:text-amber-800 dark:hover:text-white bg-amber-50 dark:bg-[#2C1E16] p-2 rounded-full transition-colors"><X size={16}/></button>
          <div className="flex justify-center mb-6">
            <div className="bg-amber-100 dark:bg-[#2C1E16] p-4 rounded-full text-amber-800 dark:text-[#C17F59]"><Lock size={40} /></div>
          </div>
          <h1 className="text-2xl font-bold text-center text-amber-950 dark:text-[#F3E8D6] mb-2">Acesso Restrito</h1>
          <p className="text-center text-stone-500 dark:text-[#E2D4C1] text-sm mb-8">Gestão interna Emural.</p>
          ...
          <form onSubmit={handleAuthenticate} className="space-y-4">
            <div>
              <label className="block text-sm font-bold text-amber-900 dark:text-[#E2D4C1] mb-1">Email do Admin</label>
              <div className="relative">
                <Mail className="absolute left-3 top-1/2 transform -translate-y-1/2 text-amber-800/50 dark:text-[#C17F59]/50" size={18} />
                <input type="email" required className="w-full pl-10 pr-4 py-3 bg-amber-50/50 dark:bg-[#2C1E16] border border-amber-200 dark:border-[#4A3B32] text-amber-950 dark:text-[#F3E8D6] rounded-xl outline-none focus:ring-2 focus:ring-amber-500 dark:focus:ring-[#C17F59] transition-colors" placeholder="admin@emural.com" value={email} onChange={(e) => setEmail(e.target.value)}/>
              </div>
            </div>
            <div>
              <label className="block text-sm font-bold text-amber-900 dark:text-[#E2D4C1] mb-1">Senha</label>
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 transform -translate-y-1/2 text-amber-800/50 dark:text-[#C17F59]/50" size={18} />
                <input type="password" required className="w-full pl-10 pr-4 py-3 bg-amber-50/50 dark:bg-[#2C1E16] border border-amber-200 dark:border-[#4A3B32] text-amber-950 dark:text-[#F3E8D6] rounded-xl outline-none focus:ring-2 focus:ring-amber-500 dark:focus:ring-[#C17F59] transition-colors" placeholder="Mínimo de 6 caracteres" value={password} onChange={(e) => setPassword(e.target.value)} />
              </div>
              {authMode === 'login' && (
                <div className="text-right mt-2">
                  <button type="button" onClick={handleResetPassword} className="text-xs text-amber-600 dark:text-[#C17F59] hover:underline font-bold transition-colors">Esqueci a senha</button>
                </div>
              )}
            </div>
            <button type="submit" className="w-full bg-amber-800 hover:bg-amber-900 dark:bg-[#C17F59] dark:hover:bg-[#A66A4B] text-white dark:text-[#2C1E16] font-bold py-3 rounded-xl transition-colors mt-4 shadow-md">
              {authMode === 'login' ? 'Entrar no Painel' : 'Criar Conta Admin'}
            </button>
          </form>
          <div className="mt-8 text-center border-t border-amber-100 dark:border-[#4A3B32] pt-6">
            <button onClick={() => { setAuthMode(authMode === 'login' ? 'register' : 'login'); setAuthError(''); setResetMessage(''); }} className="text-xs font-bold text-stone-500 hover:text-amber-800 dark:text-[#E2D4C1] dark:hover:text-white transition-colors">
  ...
  return (
    <div className={`${darkMode ? 'dark' : ''} h-full relative`}>
      ...
      <div className="flex h-[100dvh] bg-[#FFF8F0] dark:bg-[#221510] font-sans text-gray-800 dark:text-gray-100 transition-colors duration-300 overflow-hidden">
        ...
        <aside className="w-64 bg-amber-950 dark:bg-[#1A0F0A] text-amber-50 flex flex-col shadow-xl z-20 transition-colors duration-300 hidden md:flex border-r border-amber-900/50 dark:border-[#3D2C20]">
          <div className="p-6 flex items-center gap-3"><Store size={32} className="text-[#C17F59]" /><h1 className="text-2xl font-black tracking-tight">Emural Dash</h1></div>
          <nav className="flex-1 px-4 space-y-2 mt-4 overflow-y-auto">
            <button onClick={() => setActiveTab('dashboard')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-colors ${activeTab === 'dashboard' ? 'bg-amber-800 dark:bg-[#3D2C20] text-white dark:text-[#F3E8D6]' : 'hover:bg-amber-900/50 dark:hover:bg-[#2C1E16] text-amber-200/70 dark:text-[#E2D4C1]'}`}><BarChart3 size={20} /> Visão Geral</button>
            <button onClick={() => setActiveTab('products')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-colors ${activeTab === 'products' ? 'bg-amber-800 dark:bg-[#3D2C20] text-white dark:text-[#F3E8D6]' : 'hover:bg-amber-900/50 dark:hover:bg-[#2C1E16] text-amber-200/70 dark:text-[#E2D4C1]'}`}><ShoppingBag size={20} /> Catálogo</button>
            <button onClick={() => setActiveTab('inventory')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-colors ${activeTab === 'inventory' ? 'bg-amber-800 dark:bg-[#3D2C20] text-white dark:text-[#F3E8D6]' : 'hover:bg-amber-900/50 dark:hover:bg-[#2C1E16] text-amber-200/70 dark:text-[#E2D4C1]'}`}><ClipboardList size={20} /> Estoque & Prod.</button>
            <button onClick={() => setActiveTab('customers')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-colors ${activeTab === 'customers' ? 'bg-amber-800 dark:bg-[#3D2C20] text-white dark:text-[#F3E8D6]' : 'hover:bg-amber-900/50 dark:hover:bg-[#2C1E16] text-amber-200/70 dark:text-[#E2D4C1]'}`}><Users size={20} /> Clientes</button>
            <button onClick={() => setActiveTab('costs')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-colors ${activeTab === 'costs' ? 'bg-amber-800 dark:bg-[#3D2C20] text-white dark:text-[#F3E8D6]' : 'hover:bg-amber-900/50 dark:hover:bg-[#2C1E16] text-amber-200/70 dark:text-[#E2D4C1]'}`}><Calculator size={20} /> Custos & Sync</button>
            <button onClick={() => setActiveTab('reservations')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-colors ${activeTab === 'reservations' ? 'bg-amber-800 dark:bg-[#3D2C20] text-white dark:text-[#F3E8D6]' : 'hover:bg-amber-900/50 dark:hover:bg-[#2C1E16] text-amber-200/70 dark:text-[#E2D4C1]'}`}><CalendarCheck size={20} /> Entregas</button>
            <button onClick={() => setActiveTab('store_settings')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-colors ${activeTab === 'store_settings' ? 'bg-amber-800 dark:bg-[#3D2C20] text-white dark:text-[#F3E8D6]' : 'hover:bg-amber-900/50 dark:hover:bg-[#2C1E16] text-amber-200/70 dark:text-[#E2D4C1]'}`}><Store size={20} /> Loja Online</button>
            <button onClick={() => setActiveTab('network')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-colors ${activeTab === 'network' ? 'bg-amber-800 dark:bg-[#3D2C20] text-white dark:text-[#F3E8D6]' : 'hover:bg-amber-900/50 dark:hover:bg-[#2C1E16] text-amber-200/70 dark:text-[#E2D4C1]'}`}><Network size={20} /> Rede</button>
            <button onClick={() => setActiveTab('suggestions')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-colors ${activeTab === 'suggestions' ? 'bg-amber-800 dark:bg-[#3D2C20] text-white dark:text-[#F3E8D6]' : 'hover:bg-amber-900/50 dark:hover:bg-[#2C1E16] text-amber-200/70 dark:text-[#E2D4C1]'}`}><Lightbulb size={20} /> Ideias</button>
          </nav>
          <div className="mt-auto px-4 pb-6 space-y-2">
            <button onClick={() => setAppMode('storefront')} className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl transition-colors bg-amber-800 hover:bg-amber-700 dark:bg-[#C17F59] dark:hover:bg-[#A66A4B] text-white dark:text-[#2C1E16] mb-4 shadow-sm font-bold">
              <Store size={18} /> <span className="text-sm">Ver Minha Loja</span>
            </button>
            <button onClick={() => setDarkMode(!darkMode)} className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl transition-colors hover:bg-amber-900/50 dark:hover:bg-[#2C1E16] text-amber-200/70 dark:text-[#E2D4C1] hover:text-white dark:hover:text-[#F3E8D6] font-bold">
              {darkMode ? <Sun size={18} /> : <Moon size={18} />} <span className="text-sm">{darkMode ? 'Modo Claro' : 'Modo Escuro'}</span>
            </button>
            <button onClick={handleLogout} className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl transition-colors hover:bg-red-900/50 text-red-300 hover:text-red-200 font-bold">
              <LogOut size={18} /> <span className="text-sm">Sair da Conta</span>
            </button>
          </div>
        </aside>
        ...
        <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-[#3D2C20] border-t border-amber-100 dark:border-[#4A3B32] flex justify-around p-3 z-50">
           <button onClick={() => setActiveTab('dashboard')} className={`p-2 rounded-xl ${activeTab === 'dashboard' ? 'bg-amber-100 text-amber-800 dark:bg-[#2C1E16] dark:text-[#C17F59]' : 'text-stone-500 dark:text-stone-400'}`}><BarChart3 size={24}/></button>
           <button onClick={() => setActiveTab('inventory')} className={`p-2 rounded-xl ${activeTab === 'inventory' ? 'bg-amber-100 text-amber-800 dark:bg-[#2C1E16] dark:text-[#C17F59]' : 'text-stone-500 dark:text-stone-400'}`}><ClipboardList size={24}/></button>
           <button onClick={() => setActiveTab('reservations')} className={`p-2 rounded-xl ${activeTab === 'reservations' ? 'bg-amber-100 text-amber-800 dark:bg-[#2C1E16] dark:text-[#C17F59]' : 'text-stone-500 dark:text-stone-400'}`}><CalendarCheck size={24}/></button>
           <button onClick={() => setActiveTab('store_settings')} className={`p-2 rounded-xl ${activeTab === 'store_settings' ? 'bg-amber-100 text-amber-800 dark:bg-[#2C1E16] dark:text-[#C17F59]' : 'text-stone-500 dark:text-stone-400'}`}><Store size={24}/></button>
           <button onClick={() => setDarkMode(!darkMode)} className="p-2 rounded-xl text-stone-500 dark:text-stone-400">{darkMode ? <Sun size={24} /> : <Moon size={24} />}</button>
        </div>
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8 pb-32 md:pb-32">
          ...
          {activeTab === 'products' && (
            <div className="max-w-6xl mx-auto space-y-6 animate-in fade-in duration-300">
              <div className="flex justify-between items-end mb-6">
                <div>
                  <h2 className="text-3xl font-black text-amber-950 dark:text-[#F3E8D6]">Catálogo de Produtos</h2>
                  <p className="text-stone-600 dark:text-[#E2D4C1] mt-2 font-medium">Cadastre os itens da sua vitrine virtual.</p>
                </div>
              </div>

              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="bg-white dark:bg-[#3D2C20] p-6 rounded-3xl shadow-sm border border-amber-100 dark:border-[#4A3B32] h-fit transition-colors">
                  <h3 className="text-lg font-bold text-amber-950 dark:text-[#F3E8D6] mb-4 flex items-center gap-2"><Plus size={20} className="text-amber-800 dark:text-[#C17F59]" /> Adicionar Item</h3>
                  {costMetrics.costPerCookie > 0 && (
                    <div className="mb-4 p-3 bg-amber-50 dark:bg-[#2C1E16] rounded-xl border border-amber-200 dark:border-[#4A3B32]">
                      <p className="text-xs text-amber-800 dark:text-[#C17F59] font-black flex items-center gap-1 mb-1"><Lightbulb size={14}/> Sugestão de Precificação</p>
                      <p className="text-xs font-bold text-amber-900 dark:text-[#E2D4C1]">Custo: <strong>R$ {(costMetrics.costPerCookie * (newProduct.type === 'combo' ? (Number(newProduct.units) || 1) : 1)).toFixed(2)}</strong></p>
                      <p className="text-xs font-bold text-amber-900 dark:text-[#E2D4C1]">Sugestão (100%): <strong>R$ {(costMetrics.costPerCookie * (newProduct.type === 'combo' ? (Number(newProduct.units) || 1) : 1) * 2).toFixed(2)}</strong></p>
                    </div>
                  )}
                  <form onSubmit={handleAddProduct} className="space-y-4">
                    <div>
                      <label className="block text-sm font-bold text-amber-900 dark:text-[#E2D4C1] mb-1">Tipo de Item</label>
                      <select className="w-full p-3 bg-amber-50/50 dark:bg-[#2C1E16] border border-amber-200 dark:border-[#4A3B32] text-amber-950 dark:text-[#F3E8D6] rounded-xl focus:ring-2 focus:ring-amber-500 dark:focus:ring-[#C17F59] outline-none transition-colors font-medium" value={newProduct.type} onChange={e => setNewProduct({...newProduct, type: e.target.value})}>
                        <option value="single">Produto Individual (1 unid.)</option>
                        <option value="combo">Combo / Promoção</option>
                      </select>
                    </div>
                    <div>
                      <label className="block text-sm font-bold text-amber-900 dark:text-[#E2D4C1] mb-1">Nome</label>
                      <input required type="text" className="w-full p-3 bg-amber-50/50 dark:bg-[#2C1E16] border border-amber-200 dark:border-[#4A3B32] text-amber-950 dark:text-[#F3E8D6] rounded-xl focus:ring-2 focus:ring-amber-500 dark:focus:ring-[#C17F59] outline-none transition-colors font-medium" value={newProduct.name} onChange={e => setNewProduct({...newProduct, name: e.target.value})} placeholder={newProduct.type === 'combo' ? "Ex: Combo 2 Unidades" : "Ex: Bolo de Cenoura"} />
                    </div>
                    {newProduct.type === 'combo' && (
                      <div>
                        <label className="block text-sm font-bold text-amber-900 dark:text-[#E2D4C1] mb-1">Quantos itens vêm?</label>
                        <input required type="number" min="2" className="w-full p-3 bg-amber-50/50 dark:bg-[#2C1E16] border border-amber-200 dark:border-[#4A3B32] text-amber-950 dark:text-[#F3E8D6] rounded-xl focus:ring-2 focus:ring-amber-500 dark:focus:ring-[#C17F59] outline-none transition-colors font-medium" value={newProduct.units} onChange={e => setNewProduct({...newProduct, units: e.target.value})} />
                      </div>
                    )}
                    <div>
                      <label className="block text-sm font-bold text-amber-900 dark:text-[#E2D4C1] mb-1">Descrição</label>
                      <textarea className="w-full p-3 bg-amber-50/50 dark:bg-[#2C1E16] border border-amber-200 dark:border-[#4A3B32] text-amber-950 dark:text-[#F3E8D6] rounded-xl focus:ring-2 focus:ring-amber-500 dark:focus:ring-[#C17F59] outline-none transition-colors font-medium h-24 resize-none" value={newProduct.description} onChange={e => setNewProduct({...newProduct, description: e.target.value})} placeholder="Ex: Massa fofinha com..." />
                    </div>
                    <div>
                      <label className="block text-sm font-bold text-amber-900 dark:text-[#E2D4C1] mb-1">Preço de Venda (R$)</label>
                      <input required type="number" step="0.10" min="0" className="w-full p-3 bg-amber-50/50 dark:bg-[#2C1E16] border border-amber-200 dark:border-[#4A3B32] text-amber-950 dark:text-[#F3E8D6] rounded-xl focus:ring-2 focus:ring-amber-500 dark:focus:ring-[#C17F59] outline-none transition-colors font-black" value={newProduct.price} onChange={e => setNewProduct({...newProduct, price: e.target.value})} placeholder="Ex: 18.00" />
                    </div>
                    <button type="submit" className="w-full bg-amber-800 hover:bg-amber-900 dark:bg-[#C17F59] dark:hover:bg-[#A66A4B] text-white dark:text-[#2C1E16] py-3 rounded-xl transition font-bold mt-2 shadow-sm">Salvar {newProduct.type === 'combo' ? 'Combo' : 'Produto'}</button>
                  </form>
                </div>

                <div className="lg:col-span-2 flex flex-col gap-6">
                  <div>
                    <h4 className="text-md font-black text-amber-950 dark:text-[#F3E8D6] mb-3 flex items-center gap-2"><Tag size={18} className="text-amber-800 dark:text-[#C17F59]"/> Produtos Individuais</h4>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {products.filter(p => p.type === 'single' || !p.type).length === 0 ? (
                        <p className="col-span-2 text-sm text-stone-500 dark:text-[#E2D4C1] p-4 border border-dashed border-amber-200 dark:border-[#4A3B32] rounded-xl text-center font-bold">Nenhum produto individual.</p>
                      ) : products.filter(p => p.type === 'single' || !p.type).map(product => (
                        <div key={product.id} className={`bg-white dark:bg-[#3D2C20] p-5 rounded-2xl shadow-sm border ${product.isVisible === false ? 'border-gray-200 dark:border-gray-800 opacity-60' : 'border-amber-100 dark:border-[#4A3B32]'} flex justify-between items-center group transition-colors`}>
                          <div className="flex items-center gap-3">
                            <div className="bg-amber-100 dark:bg-[#2C1E16] p-3 rounded-full text-amber-800 dark:text-[#C17F59]"><Tag size={20} /></div>
                            <div>
                              <p className="font-black text-amber-950 dark:text-[#F3E8D6]">{product.name}</p>
                              <p className="text-sm font-bold text-amber-800 dark:text-[#C17F59]">R$ {(Number(product.price)||0).toFixed(2)}</p>
                            </div>
                          </div>
                          <div className="flex gap-1">
                            <button onClick={() => handleToggleProductVisibility(product.id, product.isVisible)} className={`p-2 rounded-lg transition-colors ${product.isVisible === false ? 'text-gray-400 bg-gray-100 dark:bg-gray-800 hover:text-amber-500' : 'text-amber-600 bg-amber-50 dark:bg-[#2C1E16] dark:text-[#C17F59] hover:text-amber-800'}`} title={product.isVisible === false ? "Mostrar na Loja" : "Ocultar da Loja"}>
                              {product.isVisible === false ? <EyeOff size={18} /> : <Eye size={18} />}
                            </button>
                            <button onClick={() => handleDeleteProduct(product.id)} className="p-2 text-stone-300 dark:text-stone-600 hover:text-red-500 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors"><Trash2 size={18} /></button>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <div>
                    <h4 className="text-md font-black text-amber-950 dark:text-[#F3E8D6] mb-3 flex items-center gap-2"><Layers size={18} className="text-amber-800 dark:text-[#C17F59]"/> Combos & Promoções</h4>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {products.filter(p => p.type === 'combo').length === 0 ? (
                        <p className="col-span-2 text-sm text-stone-500 dark:text-[#E2D4C1] p-4 border border-dashed border-amber-200 dark:border-[#4A3B32] rounded-xl text-center font-bold">Nenhum combo cadastrado.</p>
                      ) : products.filter(p => p.type === 'combo').map(product => (
                        <div key={product.id} className={`bg-white dark:bg-[#3D2C20] p-5 rounded-2xl shadow-sm border ${product.isVisible === false ? 'border-gray-200 dark:border-gray-800 opacity-60' : 'border-amber-100 dark:border-[#4A3B32]'} flex justify-between items-center group transition-colors`}>
                          <div className="flex items-center gap-3">
                            <div className="bg-amber-200 dark:bg-[#2C1E16] p-3 rounded-full text-amber-900 dark:text-[#C17F59]"><Layers size={20} /></div>
                            <div>
                              <p className="font-black text-amber-950 dark:text-[#F3E8D6]">{product.name}</p>
                              <div className="flex items-center gap-2">
                                <p className="text-sm font-bold text-amber-800 dark:text-[#C17F59]">R$ {(Number(product.price)||0).toFixed(2)}</p>
                                <span className="text-[10px] bg-amber-50 dark:bg-[#1A0F0A] px-2 py-0.5 rounded-full text-amber-800 dark:text-[#C17F59] font-bold border border-amber-200 dark:border-[#4A3B32]">{product.units} un</span>
                              </div>
                            </div>
                          </div>
                          <div className="flex gap-1">
          ...
          {activeTab === 'costs' && (
            <div className="max-w-6xl mx-auto space-y-6 animate-in fade-in duration-300">
              <div className="flex justify-between items-end mb-6">
                <div>
                  <h2 className="text-3xl font-black text-amber-950 dark:text-[#F3E8D6]">Custos & Planilha</h2>
                  <p className="text-stone-600 dark:text-[#E2D4C1] mt-2 font-medium">Conecte sua planilha para gerar os custos em tempo real.</p>
                </div>
              </div>
              <div className="bg-amber-900 dark:bg-[#2C1E16] p-8 rounded-3xl shadow-sm relative overflow-hidden text-white flex flex-col justify-center mb-8 border border-transparent dark:border-[#4A3B32] transition-colors">
                <div className="absolute top-0 right-0 w-64 h-64 bg-amber-800 dark:bg-black rounded-full blur-3xl -mr-20 -mt-20 opacity-50"></div>
                <div className="relative z-10 flex flex-col md:flex-row gap-8 items-center">
                  <div className="flex-1">
                    <h3 className="text-2xl font-black mb-2 flex items-center gap-2 text-white dark:text-[#F3E8D6]"><FileSpreadsheet className="text-amber-300 dark:text-[#C17F59]" /> Vínculo Planilha</h3>
                    <p className="text-amber-100 dark:text-[#E2D4C1] text-sm font-medium mb-4">Cole o link da sua planilha para atualizar o custo da massa.</p>
                    <div className="flex flex-col sm:flex-row gap-3">
                      <div className="flex-1 relative">
                        <LinkIcon className="absolute left-4 top-1/2 transform -translate-y-1/2 text-amber-800/50 dark:text-stone-500" size={18} />
                        <input type="text" placeholder="Link..." className="w-full pl-12 pr-4 py-3 rounded-xl border-none focus:outline-none focus:ring-2 focus:ring-amber-300 dark:focus:ring-[#C17F59] bg-white dark:bg-[#1A0F0A] text-amber-950 dark:text-[#F3E8D6] font-medium" value={sheetUrl} onChange={(e) => setSheetUrl(e.target.value)} />
                      </div>
                      <button onClick={handleSyncSheet} disabled={isSyncing} className="bg-amber-500 dark:bg-[#C17F59] hover:bg-amber-400 dark:hover:bg-[#A66A4B] text-amber-950 dark:text-[#2C1E16] px-6 py-3 rounded-xl font-bold transition-colors flex items-center justify-center gap-2 disabled:opacity-70 whitespace-nowrap shadow-sm">
                        <RefreshCw size={18} className={isSyncing ? "animate-spin" : ""} /> {isSyncing ? "Lendo..." : "Sincronizar"}
                      </button>
                    </div>
                    {lastSync && <p className="text-xs text-amber-200 dark:text-[#E2D4C1] mt-3 flex items-center gap-1 font-bold"><CheckCircle size={12}/> Última leitura: {lastSync}</p>}
                  </div>
                </div>
              </div>
              ...
              {ingredients.length > 0 && (
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-8">
                  <div className="lg:col-span-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div className="bg-white dark:bg-[#3D2C20] p-5 rounded-2xl shadow-sm border border-amber-100 dark:border-[#4A3B32] transition-colors">
                      <div className="flex items-center gap-2 mb-1 text-stone-500 dark:text-[#E2D4C1]"><Package size={16}/><h3 className="font-bold text-xs">Custo da Receita Base</h3></div>
                      <p className="text-xl font-black text-amber-950 dark:text-[#F3E8D6]">R$ {(Number(costMetrics.totalRecipeCost)||0).toFixed(2)}</p>
                    </div>
                    <div className="bg-white dark:bg-[#3D2C20] p-5 rounded-2xl shadow-sm border border-amber-100 dark:border-[#4A3B32] transition-colors">
                      <div className="flex items-center gap-2 mb-1 text-stone-500 dark:text-[#E2D4C1]"><Calculator size={16}/><h3 className="font-bold text-xs">Custo Unitário</h3></div>
                      <p className="text-xl font-black text-amber-950 dark:text-[#F3E8D6]">R$ {(Number(costMetrics.costPerCookie)||0).toFixed(2)}</p>
                    </div>
                    <div className="bg-white dark:bg-[#3D2C20] p-5 rounded-2xl shadow-sm border border-amber-100 dark:border-[#4A3B32] transition-colors">
                      <div className="flex items-center gap-2 mb-1 text-stone-500 dark:text-[#E2D4C1]"><DollarSign size={16} className="text-green-500"/><h3 className="font-bold text-xs text-green-700 dark:text-green-400">Lucro Médio Unitário</h3></div>
                      <p className="text-xl font-black text-green-600 dark:text-green-400">R$ {(Number(costMetrics.profit)||0).toFixed(2)}</p>
                    </div>
                    <div className="bg-white dark:bg-[#3D2C20] p-5 rounded-2xl shadow-sm border border-amber-100 dark:border-[#4A3B32] transition-colors">
                      <div className="flex items-center gap-2 mb-1 text-stone-500 dark:text-[#E2D4C1]"><TrendingUp size={16} className="text-blue-500"/><h3 className="font-bold text-xs text-blue-700 dark:text-blue-400">Margem (Base)</h3></div>
                      <p className="text-xl font-black text-blue-600 dark:text-blue-400">{(Number(costMetrics.profitMargin)||0).toFixed(1)}%</p>
                    </div>
                  </div>
                  <div className="bg-amber-50 dark:bg-[#2C1E16] p-5 rounded-2xl border border-amber-200 dark:border-[#4A3B32] transition-colors">
                    <h3 className="font-black text-amber-950 dark:text-[#F3E8D6] mb-3 text-sm">Simulador de Custos</h3>
                    <div className="space-y-3">
                      <div className="flex justify-between items-center">
                        <label className="text-xs font-bold text-amber-900 dark:text-[#E2D4C1]">Rendimento (un):</label>
                        <input type="number" className="w-16 p-1 text-center text-sm font-black border border-amber-200 dark:border-[#4A3B32] rounded bg-white dark:bg-[#3D2C20] text-amber-950 dark:text-[#F3E8D6] outline-none" value={recipeConfig.yield} onChange={(e) => setRecipeConfig({...recipeConfig, yield: Number(e.target.value) || 1})} />
                      </div>
                      <div className="flex justify-between items-center">
                        <label className="text-xs font-bold text-amber-900 dark:text-[#E2D4C1]">Venda Base(R$):</label>
                        <input type="number" step="0.5" className="w-20 p-1 text-center text-sm font-black border border-amber-200 dark:border-[#4A3B32] rounded bg-white dark:bg-[#3D2C20] text-amber-950 dark:text-[#F3E8D6] outline-none" value={recipeConfig.salePrice} onChange={(e) => setRecipeConfig({...recipeConfig, salePrice: Number(e.target.value) || 0})} />
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-white dark:bg-[#3D2C20] rounded-3xl shadow-sm border border-amber-100 dark:border-[#4A3B32] overflow-hidden transition-colors">
                <div className="p-6 border-b border-amber-100 dark:border-[#4A3B32] bg-amber-50/30 dark:bg-[#2C1E16] flex justify-between items-center">
                  <h3 className="font-bold text-amber-950 dark:text-[#F3E8D6] flex items-center gap-2"><FileSpreadsheet className="text-amber-800 dark:text-[#C17F59]" size={20} /> Insumos da Planilha</h3>
                </div>
                {ingredients.length === 0 ? (
                  <div className="p-12 text-center flex flex-col items-center justify-center bg-white dark:bg-[#3D2C20]">
                     <FileSpreadsheet className="text-amber-200 dark:text-[#4A3B32] mb-3" size={48} />
                     <p className="text-stone-500 dark:text-[#E2D4C1] font-bold text-lg">Aguardando Sincronização...</p>
                  </div>
                ) : (
                  <div className="overflow-x-auto bg-white dark:bg-[#3D2C20]">
                    <table className="w-full text-left border-collapse min-w-max">
                      <thead>
                        <tr className="bg-amber-50 dark:bg-[#2C1E16] text-amber-900 dark:text-[#C17F59] border-b border-amber-100 dark:border-[#4A3B32] text-sm">
                          <th className="p-4 font-black">Matéria Prima</th><th className="p-4 font-black">Preço Planilha</th><th className="p-4 font-black">Qtde Pacote</th><th className="p-4 font-black">Uso Receita</th><th className="p-4 font-black text-right">Custo</th>
                        </tr>
                      </thead>
                      <tbody>
                        {ingredients.map((ing) => {
                          ...
                          return (
                            <tr key={ing.id} className="border-b border-amber-50 dark:border-[#4A3B32] hover:bg-amber-50/30 dark:hover:bg-[#2C1E16] transition-colors">
                              <td className="p-4 font-black text-amber-950 dark:text-[#F3E8D6]">{ing.name}</td><td className="p-4 text-stone-600 dark:text-[#E2D4C1] font-medium">R$ {(Number(ing.bulkPrice)||0).toFixed(2)}</td><td className="p-4 text-stone-600 dark:text-[#E2D4C1] font-medium">{ing.bulkQty} {ing.unit}</td><td className="p-4 text-stone-600 dark:text-[#E2D4C1] font-medium">{ing.recipeQty} {ing.unit}</td><td className="p-4 text-right font-black text-amber-800 dark:text-[#C17F59]">R$ {ingCost.toFixed(2)}</td>
                            </tr>
                          );
                        })}
                      </tbody>
          ...
          {activeTab === 'store_settings' && (
            <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in duration-300">
              <div className="flex justify-between items-end mb-6">
                <div>
                  <h2 className="text-3xl font-black text-amber-950 dark:text-[#F3E8D6]">Loja Online</h2>
                  <p className="text-stone-600 dark:text-[#E2D4C1] mt-2 font-medium">Configure como os clientes fazem pedidos no site.</p>
                </div>
                <button onClick={() => setAppMode('storefront')} className="bg-amber-800 hover:bg-amber-900 dark:bg-[#C17F59] dark:hover:bg-[#A66A4B] text-white dark:text-[#2C1E16] font-bold py-2.5 px-5 rounded-xl transition-colors flex items-center gap-2 shadow-sm">
                  <Store size={18} /> Ver Minha Loja
                </button>
              </div>

              <div className="bg-white dark:bg-[#3D2C20] rounded-3xl shadow-sm border border-amber-100 dark:border-[#4A3B32] overflow-hidden transition-colors">
                <div className="p-6 border-b border-amber-100 dark:border-[#4A3B32] bg-amber-50/50 dark:bg-[#2C1E16]">
                  <h3 className="font-bold text-amber-950 dark:text-[#F3E8D6] flex items-center gap-2">
                    <Store className="text-amber-800 dark:text-[#C17F59]" size={20}/> Configurações Gerais
                  </h3>
                </div>
                <div className="p-6 space-y-6">
                  
                  <div className="flex items-center justify-between p-4 bg-amber-50/50 dark:bg-[#2C1E16] rounded-2xl border border-amber-200 dark:border-[#4A3B32]">
                    <div>
                      <p className="font-black text-amber-950 dark:text-[#F3E8D6] text-lg">Status do Forno</p>
                      <p className="text-sm font-medium text-stone-500 dark:text-[#E2D4C1]">Permitir pedidos agora.</p>
                    </div>
                    <button 
                      onClick={() => setStoreSettings({...storeSettings, isStoreOpen: !storeSettings.isStoreOpen})}
                      className={`relative inline-flex h-8 w-14 items-center rounded-full transition-colors focus:outline-none ${storeSettings.isStoreOpen ? 'bg-green-500' : 'bg-stone-300 dark:bg-stone-600'}`}
                    >
                  ...
                  {!storeSettings.isStoreOpen && (
                    <div className="animate-in fade-in slide-in-from-top-2">
                      <label className="block text-sm font-bold text-amber-900 dark:text-[#E2D4C1] mb-1">Mensagem de Forno Fechado</label>
                      <textarea 
                        className="w-full p-3 bg-amber-50/50 dark:bg-[#2C1E16] border border-amber-200 dark:border-[#4A3B32] rounded-xl outline-none focus:ring-2 focus:ring-amber-500 text-sm text-amber-950 dark:text-[#F3E8D6] font-medium"
                        rows="2"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <label className="block text-sm font-bold text-amber-900 dark:text-[#E2D4C1] mb-1">WhatsApp (Pedidos)</label>
                      <input 
                        type="text" 
                        className="w-full p-3 bg-amber-50/50 dark:bg-[#2C1E16] border border-amber-200 dark:border-[#4A3B32] rounded-xl outline-none focus:ring-2 focus:ring-amber-500 text-sm text-amber-950 dark:text-[#F3E8D6] font-medium"
                      ...
                      />
                      <p className="text-[10px] text-stone-500 font-bold mt-1">Inclua (+55) e DDD.</p>
                    </div>
                    <div>
                      <label className="block text-sm font-bold text-amber-900 dark:text-[#E2D4C1] mb-1">Antecedência (Dias)</label>
                      <input 
                        type="number" min="1" max="90"
                        className="w-full p-3 bg-amber-50/50 dark:bg-[#2C1E16] border border-amber-200 dark:border-[#4A3B32] rounded-xl outline-none focus:ring-2 focus:ring-amber-500 text-sm text-amber-950 dark:text-[#F3E8D6] font-black"
                        value={storeSettings.maxAdvanceDays}
                        onChange={e => setStoreSettings({...storeSettings, maxAdvanceDays: Number(e.target.value)})}
                      />
                      <p className="text-[10px] text-stone-500 font-bold mt-1">Limite do calendário.</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          {activeTab === 'customers' && (
            <div className="max-w-6xl mx-auto animate-in fade-in duration-300">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
                <div>
                  <h2 className="text-3xl font-black text-amber-950 dark:text-[#F3E8D6]">Gerir Clientes</h2>
                  <p className="text-stone-600 dark:text-[#E2D4C1] mt-1 font-medium">Alimentada pelas vendas na Visão Geral.</p>
                </div>
                <div className="flex items-center gap-2 bg-white dark:bg-[#3D2C20] px-4 py-2 rounded-xl border border-amber-200 dark:border-[#4A3B32] shadow-sm w-full sm:w-auto transition-colors">
                  <ArrowUpDown size={18} className="text-amber-800 dark:text-[#C17F59]" />
                  <select className="bg-transparent font-bold text-amber-950 dark:text-[#F3E8D6] outline-none w-full cursor-pointer" value={customerSortBy} onChange={(e) => setCustomerSortBy(e.target.value)}>
              ...
              <div className="bg-white dark:bg-[#3D2C20] rounded-3xl shadow-sm border border-amber-100 dark:border-[#4A3B32] overflow-hidden transition-colors">
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-amber-50 dark:bg-[#2C1E16] text-amber-900 dark:text-[#C17F59] border-b border-amber-100 dark:border-[#4A3B32]">
                        <th className="p-4 font-black">Nome</th><th className="p-4 font-black">Indicado por</th><th className="p-4 font-black text-center">Indicações Feitas</th><th className="p-4 font-black text-center">Nº de Pedidos</th><th className="p-4 font-black text-center">Ações</th>
                      </tr>
                    </thead>
                    <tbody>
          ...
          {activeTab === 'reservations' && (
            <div className="max-w-6xl mx-auto animate-in fade-in duration-300">
              <div className="mb-8">
                <h2 className="text-3xl font-black text-amber-950 dark:text-[#F3E8D6]">Encomendas</h2>
                <p className="text-stone-600 dark:text-[#E2D4C1] mt-2 font-medium">Ao marcá-las como "Concluídas", entram nas Vendas!</p>
              </div>
              
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                {/* Carrinho de Reservas */}
                <div className="bg-white dark:bg-[#3D2C20] p-6 rounded-3xl shadow-sm border border-amber-100 dark:border-[#4A3B32] h-fit transition-colors flex flex-col">
                  <h3 className="text-lg font-bold text-amber-950 dark:text-[#F3E8D6] mb-4 flex items-center gap-2"><Plus size={20} className="text-amber-800 dark:text-[#C17F59]" /> Nova Encomenda</h3>
                  <form onSubmit={handleFinalizeReservation} className="space-y-4 flex flex-col flex-1">
                    <div>
                      <label className="block text-xs font-bold text-amber-900 dark:text-[#E2D4C1] uppercase mb-1">Nome do Cliente</label>
                      <input list="customers-list" required type="text" className="w-full p-3 bg-amber-50/50 dark:bg-[#2C1E16] border border-amber-200 dark:border-[#4A3B32] text-amber-950 dark:text-[#F3E8D6] font-medium rounded-xl focus:ring-2 focus:ring-amber-500 outline-none transition-colors" value={newReservation.name} onChange={e => setNewReservation({...newReservation, name: e.target.value})} placeholder="Escreva ou escolha..." />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-amber-900 dark:text-[#E2D4C1] uppercase mb-1">Quem indicou? (Opcional)</label>
                      <input list="customers-list" type="text" className="w-full p-3 bg-amber-50/50 dark:bg-[#2C1E16] border border-amber-200 dark:border-[#4A3B32] text-amber-950 dark:text-[#F3E8D6] font-medium rounded-xl focus:ring-2 focus:ring-amber-500 outline-none transition-colors" value={newReservation.referredByInput} onChange={e => setNewReservation({...newReservation, referredByInput: e.target.value})} placeholder="Busque..." />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-amber-900 dark:text-[#E2D4C1] uppercase mb-1">Data de Entrega (Opcional)</label>
                      <input type="date" className="w-full p-3 bg-amber-50/50 dark:bg-[#2C1E16] border border-amber-200 dark:border-[#4A3B32] text-amber-950 dark:text-[#F3E8D6] font-medium rounded-xl focus:ring-2 focus:ring-amber-500 outline-none transition-colors [&::-webkit-calendar-picker-indicator]:dark:invert" value={newReservation.date} onChange={e => setNewReservation({...newReservation, date: e.target.value})} />
                    </div>

                    <div className="bg-amber-50 dark:bg-[#2C1E16] p-4 rounded-2xl border border-amber-200 dark:border-[#4A3B32] mt-2">
                      <label className="block text-xs font-bold text-amber-900 dark:text-[#E2D4C1] uppercase mb-1">Adicionar Item</label>
                      <select className="w-full p-3 bg-white dark:bg-[#3D2C20] border border-amber-200 dark:border-[#4A3B32] text-amber-950 dark:text-[#F3E8D6] font-bold rounded-xl outline-none focus:ring-2 focus:ring-amber-500 transition-colors mb-3 text-sm" value={newReservation.productId} onChange={e => setNewReservation({...newReservation, productId: e.target.value})}>
                        {products.length === 0 ? <option value="">Cadastre no Catálogo</option> : (
                          <>
                            <optgroup label="Produtos Individuais">{products.filter(p => p.type === 'single' || !p.type).map(p => <option key={p.id} value={p.id}>{p.name} - R$ {(Number(p.price)||0).toFixed(2)}</option>)}</optgroup>
                      ...
                      <div className="flex items-end gap-2 mb-3">
                         <div className="flex-1">
                           <label className="block text-[10px] font-black text-amber-800 dark:text-[#C17F59] uppercase tracking-wider mb-1">Quantidade</label>
                           <input type="number" min="1" className="w-full p-2.5 bg-white dark:bg-[#3D2C20] border border-amber-200 dark:border-[#4A3B32] rounded-xl outline-none text-center font-black text-amber-950 dark:text-[#F3E8D6]" value={newReservation.quantity} onChange={e => setNewReservation({...newReservation, quantity: e.target.value})} />
                         </div>
                         <button type="button" onClick={handleAddToCartReservation} className="bg-amber-800 hover:bg-amber-900 dark:bg-[#C17F59] dark:hover:bg-[#A66A4B] text-white dark:text-[#2C1E16] px-4 py-2.5 rounded-xl font-bold text-sm transition-colors shadow-sm">+ Adicionar</button>
                      </div>
                      <div>
                        <label className="block text-[10px] font-black text-amber-800 dark:text-[#C17F59] uppercase tracking-wider mb-1 mt-2">Observações</label>
                        <input type="text" className="w-full p-2.5 bg-white dark:bg-[#3D2C20] border border-amber-200 dark:border-[#4A3B32] rounded-xl outline-none text-sm font-medium text-amber-950 dark:text-[#F3E8D6]" value={newReservation.observation} onChange={e => setNewReservation({...newReservation, observation: e.target.value})} placeholder="Opcional..." />
                      </div>
                    </div>

                    {reservationCart.length > 0 && (
                      <div className="bg-white dark:bg-[#1A0F0A] p-3 rounded-xl text-sm border border-amber-200 dark:border-[#4A3B32]">
                        <p className="font-bold text-amber-950 dark:text-[#E2D4C1] mb-2">Itens na Encomenda:</p>
                        {reservationCart.map((item, i) => (
                          <div key={i} className="flex justify-between items-center text-stone-600 dark:text-stone-400 mb-1 pb-1 border-b border-amber-50 dark:border-[#3D2C20] last:border-0 font-medium">
                            <div className="flex flex-col">
                               <span>{item.quantity}x {item.productName}</span>
                               {item.observation && <span className="text-[10px] italic">Obs: {item.observation}</span>}
                            </div>
                            <div className="flex items-center gap-2">
                              <span className="font-black text-amber-900 dark:text-[#C17F59]">R$ {(Number(item.expectedRevenue)||0).toFixed(2)}</span>
                    ...
                    <button type="submit" className="w-full bg-amber-800 hover:bg-amber-900 dark:bg-[#C17F59] dark:hover:bg-[#A66A4B] text-white dark:text-[#2C1E16] py-3.5 rounded-xl transition font-bold mt-auto shadow-sm text-lg">Guardar Encomenda</button>
                  </form>
                </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                  ...
                  <div className="bg-green-50/50 dark:bg-[#1A261A] rounded-3xl shadow-sm border border-green-200 dark:border-[#2C4A2C] overflow-hidden transition-colors">
                    <div className="p-4 border-b border-green-200 dark:border-[#2C4A2C] bg-green-100/50 dark:bg-[#2C4A2C]/30 flex justify-between items-center">
                       <h3 className="font-bold text-green-900 dark:text-green-400 flex items-center gap-2"><ShoppingCart size={18}/> 🛒 Pedidos da Loja Online</h3>
                       <span className="text-xs font-bold bg-white dark:bg-[#1A261A] px-2 py-1 rounded-md text-green-700 dark:text-green-500 border border-green-100 dark:border-[#2C4A2C]">{onlineOrders.length} aguardando</span>
                    </div>
                    <div className="overflow-x-auto max-h-[300px]">
                      <table className="w-full text-left border-collapse">
                        <tbody>
                          {onlineOrders.length === 0 ? (
                            <tr><td colSpan="4" className="p-6 text-center text-sm font-bold text-stone-500 dark:text-stone-400">Nenhum pedido novo do site.</td></tr>
                          ) : onlineOrders.map((order) => (
                            <tr key={order.id} className="border-b border-green-50 dark:border-[#2C4A2C]/30 hover:bg-white dark:hover:bg-[#203320] transition-colors">
                              <td className="p-3">
                                <div className="flex flex-col gap-0.5">
                                  <p className="font-bold text-sm text-amber-950 dark:text-[#F3E8D6]">{order.customerName}</p>
                                  <p className="text-[10px] text-stone-500 font-bold">Data Pedido: {new Date(order.createdAt).toLocaleDateString('pt-BR')}</p>
                                  {order.referredByInput && <p className="text-[10px] text-amber-600 dark:text-[#C17F59] font-bold">🌟 Indicação: {order.referredByInput}</p>}
                                </div>
                              </td>
                              <td className="p-3">
                  ...
                  {/* SEÇÃO 1: PENDENTES */}
                  <div className="bg-white dark:bg-[#3D2C20] rounded-3xl shadow-sm border border-amber-100 dark:border-[#4A3B32] overflow-hidden transition-colors">
                    <div className="p-4 border-b border-amber-100 dark:border-[#4A3B32] bg-amber-50/30 dark:bg-[#2C1E16] flex justify-between items-center">
                       <h3 className="font-bold text-amber-950 dark:text-[#F3E8D6] flex items-center gap-2"><Clock size={18}/> Encomendas Pendentes</h3>
                       <div className="flex items-center gap-2">
                         <span className="text-xs font-bold bg-white dark:bg-[#3D2C20] px-2 py-1 rounded-md text-stone-500 dark:text-stone-400 hidden sm:block border border-amber-100 dark:border-[#4A3B32]">Filtrar por:</span>
                         <select className="text-xs bg-transparent outline-none font-bold text-amber-900 dark:text-[#C17F59] cursor-pointer" value={reservationSortBy} onChange={e => setReservationSortBy(e.target.value)}>
                           <option value="date-asc">Data ⬆</option>
                           <option value="date-desc">Data ⬇</option>
                           <option value="name-asc">Nome Cliente (A-Z)</option>
                           <option value="product-asc">Produto (A-Z)</option>
                    ...
                    <div className="overflow-x-auto max-h-[400px]">
                      <table className="w-full text-left border-collapse">
                        <thead>
                          <tr className="bg-amber-50 dark:bg-[#1A0F0A] text-amber-900 dark:text-[#C17F59] text-xs border-b border-amber-100 dark:border-[#4A3B32]">
                            <th className="p-3 font-black">Cliente & Pedido</th>
                            <th className="p-3 font-black text-center">Data</th>
                        ...
                        <tbody>
                          {sortedReservations.pending.length === 0 ? (
                            <tr><td colSpan="4" className="p-6 text-center text-sm font-bold text-stone-500 dark:text-stone-400">Tudo limpo por aqui! Nenhuma encomenda na fila.</td></tr>
                          ) : sortedReservations.pending.map((res) => (
                            <tr key={res.id} className="border-b border-amber-50 dark:border-[#4A3B32] hover:bg-amber-50/30 dark:hover:bg-[#2C1E16] transition-colors">
                              <td className="p-3">
                                <p className="font-black text-sm text-amber-950 dark:text-[#F3E8D6]">{res.name}</p>
                                
                                <div className="flex flex-col gap-1 mt-0.5">
                                  <p className="text-xs text-stone-600 dark:text-[#E2D4C1] font-bold">{res.quantity}x {res.productName}</p>
                                  {res.observation && (
                                    <span 
                                      className="inline-flex items-center gap-1 w-fit bg-amber-50 dark:bg-[#1A0F0A] text-amber-800 dark:text-[#C17F59] px-2 py-1 rounded-md text-[10px] cursor-help font-bold border border-amber-200 dark:border-[#4A3B32]"
                              ...
                              <td className="p-3 text-center text-xs font-black text-amber-800 dark:text-[#C17F59]">{res.date ? new Date(res.date).toLocaleDateString('pt-BR', { timeZone: 'UTC' }) : '-'}</td>
                              <td className="p-3 text-right text-sm font-black text-amber-950 dark:text-[#F3E8D6]">R$ {(Number(res.expectedRevenue)||0).toFixed(2)}</td>
                              <td className="p-3 text-center flex items-center justify-center gap-1 mt-2">
                                <button onClick={() => handleUpdateReservationStatus(res.id, 'completed')} className="text-green-600 dark:text-green-500 hover:bg-green-100 dark:hover:bg-green-900/20 p-1.5 rounded transition-colors" title="Concluir (Envia Venda)"><CheckCircle size={18}/></button>
                                <button onClick={() => {
                                  const referrer = customers.find(c => c.id === res.referredBy);
                                  setEditingReservation({...res, referredByInput: referrer ? referrer.name : ''});
                                }} className="text-amber-600 hover:bg-amber-100 dark:text-[#C17F59] dark:hover:bg-[#2C1E16] p-1.5 rounded transition-colors" title="Editar"><Edit size={16}/></button>
                                <button onClick={() => handleUpdateReservationStatus(res.id, 'cancelled')} className="text-red-500 hover:bg-red-100 dark:hover:bg-red-900/20 p-1.5 rounded transition-colors" title="Cancelar"><XCircle size={18}/></button>
                              </td>
                            </tr>
                          ))}
                        </tbody>
                  ...
                  {/* SEÇÃO 2: HISTÓRICO */}
                  <div className="bg-white dark:bg-[#3D2C20] rounded-3xl shadow-sm border border-amber-100 dark:border-[#4A3B32] overflow-hidden transition-colors opacity-80 hover:opacity-100">
                    <div className="p-3 border-b border-amber-100 dark:border-[#4A3B32] bg-amber-50/30 dark:bg-[#2C1E16]">
                       <h3 className="font-bold text-stone-600 dark:text-stone-400 flex items-center gap-2 text-sm"><History size={16}/> Histórico de Encomendas</h3>
                    </div>
                    <div className="overflow-x-auto max-h-[250px]">
                      <table className="w-full text-left border-collapse">
                        <tbody>
                          {sortedReservations.past.length === 0 ? (
                            <tr><td className="p-4 text-center text-xs font-bold text-stone-400">Nenhum histórico.</td></tr>
                          ) : sortedReservations.past.map((res) => (
                            <tr key={res.id} className="border-b border-amber-50 dark:border-[#4A3B32] hover:bg-amber-50/30 dark:hover:bg-[#2C1E16] transition-colors">
                              <td className="p-3">
                                <p className="font-bold text-xs text-amber-950 dark:text-[#F3E8D6]">{res.name} <span className="text-stone-500 dark:text-stone-400">({res.quantity}x {res.productName})</span></p>
                              </td>
                              <td className="p-3 text-center">
                                {res.status === 'completed' ? <span className="text-[10px] font-black text-green-700 dark:text-green-400 bg-green-100 dark:bg-green-900/40 px-2 py-0.5 rounded">Concluída</span> : <span className="text-[10px] font-black text-red-700 dark:text-red-400 bg-red-100 dark:bg-red-900/40 px-2 py-0.5 rounded">Cancelada</span>}
                              </td>
                              <td className="p-3 text-right">
                                <button onClick={() => {
                                  const referrer = customers.find(c => c.id === res.referredBy);
                                  setEditingReservation({...res, referredByInput: referrer ? referrer.name : ''});
                                }} className="text-stone-400 hover:text-amber-800 dark:hover:text-[#C17F59] p-1 rounded" title="Editar Histórico"><Edit size={14}/></button>
                                <button onClick={() => handleDeleteReservation(res.id)} className="text-stone-400 hover:text-red-500 p-1 rounded" title="Excluir Definitivamente"><Trash2 size={14}/></button>
                              </td>
                            </tr>
                          ))}
                        </tbody>
          ...
          {activeTab === 'network' && (
            <div className="max-w-5xl mx-auto h-full flex flex-col animate-in fade-in duration-300">
              <div className="mb-6">
                <h2 className="text-3xl font-black text-amber-950 dark:text-[#F3E8D6]">Rede de Indicações</h2>
                <p className="text-stone-600 dark:text-[#E2D4C1] mt-2 font-medium">Veja a árvore de conexões: descubra quem iniciou a corrente e clique para expandir.</p>
              </div>
              <div className="flex-1 bg-white dark:bg-[#3D2C20] p-8 rounded-3xl shadow-sm border border-amber-100 dark:border-[#4A3B32] overflow-auto transition-colors">
                <div className="min-w-max pb-10">
                  {rootCustomers.length === 0 ? (
                    <p className="text-stone-500 dark:text-stone-400 font-bold text-center mt-10">Nenhum cliente cadastrado ainda.</p>
                  ) : (
          ...
          {activeTab === 'suggestions' && (
            <div className="max-w-6xl mx-auto animate-in fade-in duration-300">
              <div className="flex justify-between items-end mb-8">
                <div>
                  <h2 className="text-3xl font-black text-amber-950 dark:text-[#F3E8D6]">Ideias & Sugestões</h2>
                  <p className="text-stone-600 dark:text-[#E2D4C1] mt-2 font-medium">Anote feedbacks e processe sugestões enviadas pelos clientes na loja.</p>
                </div>
              </div>

              {/* IDEIAS PENDENTES DA LOJA ONLINE */}
              {onlineSuggestions.length > 0 && (
                 <div className="bg-blue-50/80 dark:bg-[#1A1C29] rounded-3xl shadow-sm border border-blue-200 dark:border-[#2C334A] p-6 mb-8 transition-colors">
                    <h3 className="text-xl font-bold text-blue-900 dark:text-blue-400 flex items-center gap-2 mb-4">
                      <MessageSquarePlus size={22}/> Feedback da Loja ({onlineSuggestions.length})
                    </h3>
                    <div className="space-y-4">
                       {onlineSuggestions.map(pubSug => {
                          ...
                          return (
                             <div key={pubSug.id} className="bg-white dark:bg-[#252838] p-4 rounded-2xl border border-blue-100 dark:border-[#2C334A] flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
                                <div>
                                   <span className="text-[10px] font-black uppercase tracking-wider text-blue-500 mb-1 block">{(pubSug.name || 'Anónimo').trim()} enviou:</span>
                                   <p className="text-amber-950 dark:text-[#F3E8D6] font-bold">"{pubSug.text}"</p>
                                </div>
                                <div className="flex flex-col items-end gap-2 w-full md:w-auto">
                                   {match ? (
                                      <div className="flex flex-col items-end w-full">
                                         <p className="text-[10px] text-amber-600 dark:text-[#C17F59] font-bold mb-1">⚠️ Parecido com: "{match.text}"</p>
                                         <div className="flex gap-2 w-full md:w-auto">
                                            <button onClick={() => handleApprovePublicSuggestion(pubSug, false)} className="flex-1 text-xs bg-amber-50 dark:bg-[#1A0F0A] hover:bg-amber-100 text-stone-700 dark:text-stone-300 font-bold py-2 px-3 rounded-lg transition-colors border border-amber-200 dark:border-[#4A3B32]">Criar Nova</button>
                                            <button onClick={() => handleApprovePublicSuggestion(pubSug, true, match.id)} className="flex-1 text-xs bg-amber-600 hover:bg-amber-700 text-white font-bold py-2 px-3 rounded-lg transition-colors flex items-center gap-1 justify-center"><ThumbsUp size={14}/> Votar (+1)</button>
                                      ...
                                      <div className="flex gap-2 w-full md:w-auto">
                                         <button onClick={() => handleApprovePublicSuggestion(pubSug, false)} className="flex-1 text-xs bg-amber-600 hover:bg-amber-700 text-white font-bold py-2 px-4 rounded-lg transition-colors">Aprovar como Nova</button>
                                         <button onClick={() => handleRejectPublicSuggestion(pubSug.id)} className="text-xs bg-red-50 dark:bg-red-900/20 hover:bg-red-100 text-red-500 py-2 px-3 rounded-lg transition-colors"><Trash2 size={16}/></button>
                                      </div>
                                   )}
                                </div>
              ...
              <div className="bg-white dark:bg-[#3D2C20] p-4 rounded-xl shadow-sm border border-amber-100 dark:border-[#4A3B32] mb-8 flex flex-col gap-4 transition-colors">
                <form onSubmit={handleAddSuggestion} className="flex flex-col md:flex-row gap-4 items-end">
                  <div className="flex-1 w-full">
                    <label className="block text-sm font-bold text-amber-950 dark:text-[#F3E8D6] mb-1">Descreva a ideia (Admin)...</label>
                    <input required type="text" className="w-full p-2 bg-amber-50 dark:bg-[#2C1E16] border border-amber-200 dark:border-[#4A3B32] text-amber-950 dark:text-[#F3E8D6] rounded-lg focus:ring-2 focus:ring-amber-500 dark:focus:ring-[#C17F59] outline-none transition-colors font-medium" value={newSuggestion.text} onChange={e => setNewSuggestion({...newSuggestion, text: e.target.value})} placeholder="Ex: Produto sabor Limão Siciliano" />
                  </div>
                  <div className="w-full md:w-48">
                    <label className="block text-sm font-bold text-amber-950 dark:text-[#F3E8D6] mb-1">Categoria</label>
                    <select className="w-full p-2 bg-amber-50 dark:bg-[#2C1E16] border border-amber-200 dark:border-[#4A3B32] text-amber-950 dark:text-[#F3E8D6] rounded-lg focus:ring-2 focus:ring-amber-500 dark:focus:ring-[#C17F59] outline-none transition-colors font-medium" value={newSuggestion.type} onChange={e => setNewSuggestion({...newSuggestion, type: e.target.value})}>
                      <option value="flavor">🍪 Novo Sabor</option>
                      <option value="product">📦 Novo Produto</option>
                      <option value="improvement">⚙️ Melhoria</option>
                    </select>
                  </div>
                  <button type="submit" className="w-full md:w-auto bg-amber-800 dark:bg-[#C17F59] dark:hover:bg-[#A66A4B] text-white dark:text-[#2C1E16] px-6 py-2 rounded-lg hover:bg-amber-900 transition font-bold flex items-center justify-center gap-2 h-[42px]"><Plus size={18} /> Adicionar</button>
                </form>
                {suggestionMatchContext && (
                  <div className="bg-amber-100 dark:bg-[#2C1E16] border border-amber-200 dark:border-[#4A3B32] p-4 rounded-lg flex flex-col sm:flex-row items-center justify-between gap-4 mt-2 transition-colors">
                    <p className="text-amber-900 dark:text-[#E2D4C1] text-sm"><strong>Ideia similar encontrada:</strong> "{suggestionMatchContext.match.text}".<br/>Deseja apenas adicionar um voto nela?</p>
                    <div className="flex gap-2 w-full sm:w-auto">
                      <button onClick={proceedAddSuggestion} className="flex-1 sm:flex-none px-4 py-2 text-sm font-bold text-amber-900 dark:text-[#F3E8D6] bg-white dark:bg-[#3D2C20] border border-amber-200 dark:border-[#4A3B32] hover:bg-amber-50 dark:hover:bg-[#4A3B32] rounded-lg transition">Criar Nova</button>
                      <button onClick={acceptSuggestionMatch} className="flex-1 sm:flex-none px-4 py-2 text-sm font-bold text-white dark:text-[#2C1E16] bg-amber-800 dark:bg-[#C17F59] hover:bg-amber-900 dark:hover:bg-[#A66A4B] rounded-lg transition flex items-center justify-center gap-2"><ThumbsUp size={16} /> Votar na Existente</button>
                    </div>
                  </div>
                )}
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {['flavor', 'product', 'improvement'].map((catType) => (
                  <div key={catType} className={`rounded-2xl p-4 border transition-colors ${catType === 'flavor' ? 'bg-orange-50/50 dark:bg-[#2C1E16] border-orange-100 dark:border-[#4A3B32]' : catType === 'product' ? 'bg-amber-50/50 dark:bg-[#2C1E16] border-amber-100 dark:border-[#4A3B32]' : 'bg-stone-50/50 dark:bg-[#2C1E16] border-stone-200 dark:border-[#4A3B32]'}`}>
                    <h3 className={`font-black mb-4 flex items-center gap-2 ${catType === 'flavor' ? 'text-orange-800 dark:text-orange-400' : catType === 'product' ? 'text-amber-800 dark:text-[#C17F59]' : 'text-stone-800 dark:text-[#E2D4C1]'}`}>
                    ...
                    <div className="space-y-3">
                      {suggestions.filter(s => s.type === catType).sort((a,b) => (b.isFavorite ? 1 : 0) - (a.isFavorite ? 1 : 0) || (b.votes || 0) - (a.votes || 0)).map(s => (
                        <div key={s.id} className={`bg-white dark:bg-[#3D2C20] p-3 rounded-lg shadow-sm border ${s.isFavorite ? 'border-amber-400 dark:border-[#C17F59] ring-1 ring-amber-400 dark:ring-[#C17F59]' : 'border-amber-50 dark:border-[#4A3B32]'} flex flex-col gap-2 transition-all`}>
                          <div className="flex justify-between items-start gap-2">
                            <div className="flex-1">
                              <span className="text-amber-950 dark:text-[#F3E8D6] text-sm font-bold">{s.text}</span>
                              <span className="text-[10px] text-stone-500 dark:text-stone-400 block mt-1 font-medium">Sugerido por: {s.author || 'Admin'}</span>
                            </div>
                            <button onClick={() => handleToggleFavorite(s.id)} className={`${s.isFavorite ? 'text-amber-400 dark:text-[#C17F59]' : 'text-stone-300 dark:text-stone-600 hover:text-amber-400 dark:hover:text-[#C17F59]'} transition-colors`}><Star size={18} fill={s.isFavorite ? 'currentColor' : 'none'} /></button>
                          </div>
                          <div className="flex justify-between items-center mt-1 border-t border-amber-50 dark:border-[#4A3B32] pt-2">
                            <button onClick={() => handleUpvoteSuggestion(s.id)} className="flex items-center gap-1 text-xs font-black text-stone-500 dark:text-stone-400 hover:text-amber-800 dark:hover:text-[#C17F59] bg-amber-50 dark:bg-[#2C1E16] hover:bg-amber-100 dark:hover:bg-[#4A3B32] px-2 py-1 rounded transition-colors border border-amber-100 dark:border-[#4A3B32] hover:border-amber-300 dark:hover:border-[#C17F59]"><ThumbsUp size={14} /> {s.votes || 0}</button>
                            <button onClick={() => handleDeleteSuggestion(s.id)} className="text-stone-400 dark:text-stone-500 hover:text-red-500 dark:hover:text-red-400 transition-colors p-1 rounded hover:bg-red-50 dark:hover:bg-red-900/20"><Trash2 size={16} /></button>
                          </div>
                        </div>
                      ))}
                    </div>
          ...
          <div className="fixed bottom-20 md:bottom-8 left-1/2 transform -translate-x-1/2 z-40 flex flex-col items-center">
             {showFooterDetails && (
               <div className="mb-2 bg-amber-950 dark:bg-black border border-amber-900 dark:border-[#3D2C20] shadow-2xl rounded-2xl p-5 text-sm text-[#F3E8D6] w-80 animate-in fade-in slide-in-from-bottom-2">
                 <div className="flex items-center gap-2 font-black text-white mb-3 border-b border-amber-900 dark:border-[#4A3B32] pb-2">
                   <Info size={16} className="text-amber-400 dark:text-[#C17F59]" /> Resumo Financeiro
                 </div>
                 <div className="flex justify-between items-center mb-2">
                    <span className="text-[#E2D4C1] font-medium">Receita Total <span className="text-[10px] block">Todas as Vendas</span></span>
                    <span className="text-green-400 font-black">R$ {(Number(globalMetrics.totalRevenue)||0).toFixed(2)}</span>
                 </div>
                 <div className="flex justify-between items-center mb-2">
                    <span className="text-[#E2D4C1] font-medium">Custo Histórico <span className="text-[10px] block">Ficha Técnica ({(Number(globalMetrics.totalCookiesSold)||0)} un.)</span></span>
                    <span className="text-red-400 font-black">-R$ {(Number(globalMetrics.totalEstimatedCost)||0).toFixed(2)}</span>
                 </div>
                 <div className="flex justify-between mt-2 pt-2 border-t border-amber-900 dark:border-[#4A3B32] mb-3">
                    <span className="font-bold text-white">Lucro Real (Histórico)</span>
                    <span className="text-amber-400 dark:text-[#C17F59] font-black text-lg">R$ {(Number(globalMetrics.totalEstimatedProfit)||0).toFixed(2)}</span>
                 </div>
                 <div className="bg-black/30 dark:bg-[#1A0F0A] rounded-xl p-3 border border-amber-900/50 dark:border-[#3D2C20]">
                    <span className="text-[#E2D4C1] text-xs block mb-1 font-medium">Custo para bater +1 receita hoje (Mercado):</span>
                    <span className="text-red-400 font-black block text-right">R$ {(Number(missingCostForOneBatch)||0).toFixed(2)}</span>
                 </div>
               </div>
             )}
             ...
             <button 
               ...
               className="bg-amber-950 dark:bg-[#1A0F0A] backdrop-blur-md border border-amber-900/50 dark:border-[#3D2C20] shadow-[0_10px_40px_rgba(0,0,0,0.3)] hover:border-amber-600/50 dark:hover:border-[#C17F59]/50 rounded-2xl px-6 py-3 flex items-center justify-between gap-4 md:gap-8 text-white w-[90vw] max-w-lg md:w-auto whitespace-nowrap overflow-x-auto transition-all cursor-help"
             >
               <div className="flex flex-col items-center">
                 <span className="text-[10px] uppercase text-[#E2D4C1] font-black tracking-wider">Faturamento</span>
                 <span className="font-black text-green-400">R$ {(Number(globalMetrics.totalRevenue)||0).toFixed(2)}</span>
               </div>
               <div className="h-6 w-px bg-amber-900/50 dark:bg-[#3D2C20]"></div>
               <div className="flex flex-col items-center">
                 <span className="text-[10px] uppercase text-[#E2D4C1] font-black tracking-wider">Custo Reposição</span>
                 <span className="font-black text-red-400">-R$ {(Number(missingCostForOneBatch)||0).toFixed(2)}</span>
               </div>
               <div className="h-6 w-px bg-amber-900/50 dark:bg-[#3D2C20]"></div>
               <div className="flex flex-col items-center">
                 <span className="text-[10px] uppercase text-[#E2D4C1] font-black tracking-wider">Lucro Líquido</span>
                 <span className="font-black text-amber-400 dark:text-[#C17F59] flex items-center gap-1">R$ {(Number(globalMetrics.totalEstimatedProfit)||0).toFixed(2)} <Info size={12} className="text-[#E2D4C1] hidden md:block" /></span>
               </div>
             </button>
          </div>
          ...
          {editingCustomer && (
            <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
              <div className="bg-white dark:bg-[#3D2C20] rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-200 border border-amber-100 dark:border-[#4A3B32]">
                <div className="p-4 border-b border-amber-100 dark:border-[#4A3B32] flex justify-between items-center bg-amber-50 dark:bg-[#2C1E16]">
                  <h3 className="font-bold text-amber-950 dark:text-[#F3E8D6] flex items-center gap-2"><Edit size={18} className="text-amber-800 dark:text-[#C17F59]"/> Editar Cliente</h3>
                  <button onClick={() => setEditingCustomer(null)} className="text-stone-400 hover:text-red-500"><X size={20} /></button>
                </div>
                <form onSubmit={handleSaveEditCustomer} className="p-6 space-y-4">
                  <div className="bg-amber-100 dark:bg-[#2C1E16] border border-amber-200 dark:border-[#4A3B32] p-3 rounded-xl mb-4">
                     <p className="text-[10px] text-amber-900 dark:text-[#C17F59] font-bold leading-tight">
                  ...
                  <div>
                    <label className="block text-xs font-black text-stone-500 dark:text-stone-400 uppercase mb-1">Nome</label>
                    <input type="text" required className="w-full p-2.5 bg-amber-50 dark:bg-[#1A0F0A] border border-amber-200 dark:border-[#4A3B32] rounded-xl outline-none text-amber-950 dark:text-[#F3E8D6] font-medium" value={editingCustomer.name} onChange={e => setEditingCustomer({...editingCustomer, name: e.target.value})} />
                  </div>
                  <div>
                    <label className="block text-xs font-black text-stone-500 dark:text-stone-400 uppercase mb-1">Qtd. Compras</label>
                    <input type="number" min="0" required className="w-full p-2.5 bg-amber-50 dark:bg-[#1A0F0A] border border-amber-200 dark:border-[#4A3B32] rounded-xl outline-none text-amber-950 dark:text-[#F3E8D6] font-medium" value={editingCustomer.purchases} onChange={e => setEditingCustomer({...editingCustomer, purchases: e.target.value})} />
                  </div>
                  <div>
                    <label className="block text-xs font-black text-stone-500 dark:text-stone-400 uppercase mb-1">Quem indicou? (Opcional)</label>
                    <input list="customers-list" type="text" className="w-full p-2.5 bg-amber-50 dark:bg-[#1A0F0A] border border-amber-200 dark:border-[#4A3B32] rounded-xl outline-none text-amber-950 dark:text-[#F3E8D6] font-medium" value={editingCustomer.referredByInput !== undefined ? editingCustomer.referredByInput : ''} onChange={e => setEditingCustomer({...editingCustomer, referredByInput: e.target.value})} placeholder="Busque ou apague para remover..." />
                  </div>
                  <button type="submit" className="w-full bg-amber-800 dark:bg-[#C17F59] dark:hover:bg-[#A66A4B] text-white dark:text-[#2C1E16] font-bold py-3.5 rounded-xl mt-2 hover:bg-amber-900 shadow-sm transition-colors">Salvar Alterações</button>
                </form>
              </div>
            </div>
          )}
          ...
          {editingReservation && (
            <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
              <div className="bg-white dark:bg-[#3D2C20] rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200 border border-amber-100 dark:border-[#4A3B32]">
                <div className="p-4 border-b border-amber-100 dark:border-[#4A3B32] flex justify-between items-center bg-amber-50 dark:bg-[#2C1E16]">
                  <h3 className="font-bold text-amber-950 dark:text-[#F3E8D6] flex items-center gap-2"><Edit size={18} className="text-amber-800 dark:text-[#C17F59]"/> Editar Encomenda</h3>
                  <button onClick={() => setEditingReservation(null)} className="text-stone-400 hover:text-red-500"><X size={20} /></button>
                </div>
                <form onSubmit={handleSaveEditReservation} className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                  <div>
                    <label className="block text-xs font-black text-stone-500 dark:text-stone-400 uppercase mb-1">Cliente</label>
                    <input type="text" required className="w-full p-2.5 bg-amber-50 dark:bg-[#1A0F0A] border border-amber-200 dark:border-[#4A3B32] rounded-xl outline-none text-amber-950 dark:text-[#F3E8D6] font-medium" value={editingReservation.name} onChange={e => handleEditReservationChange('name', e.target.value)} />
                  </div>
                  <div>
                    <label className="block text-xs font-black text-stone-500 dark:text-stone-400 uppercase mb-1">Quem indicou? (Opcional)</label>
                    <input list="customers-list" type="text" className="w-full p-2.5 bg-amber-50 dark:bg-[#1A0F0A] border border-amber-200 dark:border-[#4A3B32] rounded-xl outline-none text-amber-950 dark:text-[#F3E8D6] font-medium" value={editingReservation.referredByInput !== undefined ? editingReservation.referredByInput : ''} onChange={e => handleEditReservationChange('referredByInput', e.target.value)} placeholder="Busque ou apague para remover..." />
                  </div>
                  <div className="flex gap-3">
                    <div className="w-2/3">
                      <label className="block text-xs font-black text-stone-500 dark:text-stone-400 uppercase mb-1">Produto</label>
                      <select className="w-full p-2.5 bg-amber-50 dark:bg-[#1A0F0A] border border-amber-200 dark:border-[#4A3B32] rounded-xl outline-none text-sm text-amber-950 dark:text-[#F3E8D6] font-medium" value={editingReservation.productId} onChange={e => handleEditReservationChange('productId', e.target.value)}>
                        <option value="avulso">Produto Avulso</option>
                        {products.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                      </select>
                    </div>
                    <div className="w-1/3">
                      <label className="block text-xs font-black text-stone-500 dark:text-stone-400 uppercase mb-1">Qtd</label>
                      <input type="number" min="1" required className="w-full p-2.5 bg-amber-50 dark:bg-[#1A0F0A] border border-amber-200 dark:border-[#4A3B32] rounded-xl outline-none text-center text-amber-950 dark:text-[#F3E8D6] font-black" value={editingReservation.quantity} onChange={e => handleEditReservationChange('quantity', e.target.value)} />
                    </div>
                  </div>
                  <div className="flex gap-3">
                    <div className="w-1/2">
                      <label className="block text-xs font-black text-stone-500 dark:text-stone-400 uppercase mb-1">Data Entrega</label>
                      <input type="date" className="w-full p-2.5 bg-amber-50 dark:bg-[#1A0F0A] border border-amber-200 dark:border-[#4A3B32] rounded-xl outline-none text-amber-950 dark:text-[#F3E8D6] font-medium [&::-webkit-calendar-picker-indicator]:dark:invert" value={editingReservation.date} onChange={e => handleEditReservationChange('date', e.target.value)} />
                    </div>
                    <div className="w-1/2">
                      <label className="block text-xs font-black text-stone-500 dark:text-stone-400 uppercase mb-1">Status</label>
                      <select className="w-full p-2.5 bg-amber-50 dark:bg-[#1A0F0A] border border-amber-200 dark:border-[#4A3B32] rounded-xl outline-none text-sm text-amber-950 dark:text-[#F3E8D6] font-bold" value={editingReservation.status} onChange={e => handleEditReservationChange('status', e.target.value)}>
                  ...
                  <div>
                    <label className="block text-xs font-black text-stone-500 dark:text-stone-400 uppercase mb-1">Observações</label>
                    <input type="text" className="w-full p-2.5 bg-amber-50 dark:bg-[#1A0F0A] border border-amber-200 dark:border-[#4A3B32] rounded-xl outline-none text-amber-950 dark:text-[#F3E8D6] font-medium" value={editingReservation.observation || ''} onChange={e => handleEditReservationChange('observation', e.target.value)} />
                  </div>
                  <div>
                    <label className="block text-xs font-black text-stone-500 dark:text-stone-400 uppercase mb-1">Valor Cobrado (R$)</label>
                    <input type="number" step="0.5" required className="w-full p-2.5 bg-amber-100 dark:bg-[#2C1E16] border border-amber-200 dark:border-[#C17F59]/50 rounded-xl outline-none font-black text-amber-800 dark:text-[#C17F59]" value={editingReservation.expectedRevenue} onChange={e => handleEditReservationChange('expectedRevenue', e.target.value)} />
                  </div>
                  <button type="submit" className="w-full bg-amber-800 dark:bg-[#C17F59] dark:hover:bg-[#A66A4B] text-white dark:text-[#2C1E16] font-bold py-3.5 rounded-xl mt-2 hover:bg-amber-900 shadow-sm transition-colors">Salvar Alterações</button>
                </form>
              </div>
